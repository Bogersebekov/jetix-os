...
## 3.5 Что именно мы развиваем: интеллект = горизонт × ответственность

> **🔖 intelligence-frame (встроено 2026-05-30, DRAFT R1).** [src: Концепт 4 · METHOD-V2 §5/§6 · audit
> Таблица 2 «intelligence-frame» P1]

Когда я говорю «развивать самого себя», под «интеллектом» здесь имеется в виду не raw-IQ, а две
развиваемые переменные:

- **🔭 Горизонт прогноза** — насколько далеко ты можешь просчитать/продумать последствия. Метод и
  «метод выбора методов» (§2) тренируют именно это; AI-substrate (exocortex) **расширяет горизонт**.
- **🛡️ Ответственность / надёжность** — насколько на тебя можно положиться надолго. Это репутация и
  способность держать обязательства.

Почему это важно (и почему это не абстракция): именно **(длинный горизонт) × (ответственность)**
позволяют людям **кооперироваться надолго** — а долгая кооперация и есть то, на чём держится вся сеть
(подробнее — в P-4, «почему это работает»). Ключевое: обе оси **развиваемы** (не врождённый дар) — это
skill, который тренируется, а не каста «умных vs глупых».
...
## 6. Two-sided: тренер заряжен учениками, ученик — тренером

> **🔖 two-sided платформа (встроено 2026-05-30, DRAFT R1).** [src: two-sided-talent-platform Tier B,
> O-264 · audit Таблица 2 «two-sided платформа» P1]

Метод работает не в вакууме — он работает в **двусторонней среде.** С одной стороны — мастера-профи; с
другой — заряженные ученики, которым важно их дело. Обе стороны выигрывают **от наличия друг друга**:
профи получают усиление, защиту IP и заряженных людей рядом; ученики — тренера, среду и команду. Это
**love-economy** (взаимная вера и тяга), а не извлечение одной стороны из другой. Механика «тренер ↔
ученик» масштабируется через кланы (P-9) и события/хакатоны.
