---
type: niche-index
niche: meta
updated: 2026-04-16
---
# Niche: meta

Срез wiki о самой системе: агенты, их стратегии, процессы, self-improvement.

## Что сюда относится
- Архитектурные решения Jetix OS
- Стратегии агентов (system prompt learning)
- Пайплайны (voice-notes, ingest, lint)
- Меж-агентные паттерны и инциденты

## Агенты
- manager
- system-admin
- inbox-processor
- crazy-agent
- meta-agent
- personal-assistant

## Pages

### Ideas
- [[ideas/writing-is-telepathy]] — фреймворк для контент-маркетинга
- [[ideas/think-do-feedback-loop]] — принцип культуры работы

### Concepts
- [[concepts/writing-as-telepathy]] — образ автора = образ читателя
- [[concepts/think-do-feedback-loop]] — два режима мозга

### Sources
- [[sources/2026-04-16-writing-is-telepathy]]
- [[sources/2026-04-16-think-do-feedback-loop]]
