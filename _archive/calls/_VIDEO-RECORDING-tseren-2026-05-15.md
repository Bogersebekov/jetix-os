---
title: Видео под Цэрэна v2 — 6-block scenario + Antigravity walkthrough
date: 2026-05-15
type: outreach-video-scenario
status: ready-to-record
target: Цэрэн Цэрэнов (@TserenTserenov) — Managing Partner МИМ
purpose: первое касание → выход на 30-60 мин созвон
companion: _VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md
supersedes: outreach/video-script-tseren-2026-05-12.md
duration_target: 5-7 min
recording_format: talking head + Antigravity screen-share
language: russian
notion_view: https://www.notion.so/3602496333bf815cb7e0cfc5f433046f
F: F2
G: outreach-video-applied-now
R: refuted_if_no_response_within_14d_OR_recording_diverges_significantly
---

# 🎬 Видео под Цэрэна v2 — 6-block scenario + Antigravity walkthrough

> **Контекст.** Получатель — Цэрэн Цэрэнов (Managing Partner **МИМ — Мастерская Инженеров-Менеджеров**, 11 лет с Левенчуком). Канал: TG `@TserenTserenov`. Длина: 5-7 мин talking head + Antigravity screen-share. Этап воронки: первое касание → созвон 30-60 мин.
>
> **Tabs prep:** [_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md](_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md) — открыть ДО записи.
>
> **Bridge к Цэрэну:** МИМ = «Мастерская» в названии. Моё «мастерская по работе с информацией» — рядом. Это хребет видео.

---

## §0 Общая рамка

- **Как держусь:** прямо, без воды, talking points (не verbatim), «есть серьёзное предложение», не «можно я расскажу»
- **Что показываем экраном:** Antigravity с открытыми файлами (см. [Antigravity чек-лист](_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md))
- **На чём вырубаемся:** я говорю >80% времени; demo «я нажимаю кнопки»; перечисление 12 Constitutional rules; извинения «извини что длинно»

---

## §1 СТРУКТУРА ВИДЕО — 6 блоков (5-7 мин total)

### Блок 1 — Приветствие + контекст «почему именно тебе» + roadmap видео (≈45 сек)

**Почему блок нужен:** personal touch + ответ на «почему я получил это видео» сразу + structure preview, чтобы получатель знал что его ждёт.

**Смысловые ключи:**

**1a. Приветствие + основная идея:**
- «Цэрэн, привет. Это Руслан из Берлина.»
- «Строю **систему по работе с информацией** — для предпринимателей и профессионалов».

**1b. Почему именно тебе (не с балды):**
- «Начал искать людей которые занимаются похожими делами — кому это будет интересно».
- «Наткнулся на тебя — изучил твою работу (МИМ, ШСМ, Телеграм-канал, публикации).»
- «Мне кажется, тебе это будет интересно и полезно — потому что мы про одно».

**1c. Структура видео (roadmap в самом начале):**
- «За 5-7 минут расскажу:»
  - **Кто я** и что сделал
  - **Какие инструменты** у меня уже есть
  - **Какое у меня к тебе предложение**
- «Потом — приглашаю на 30-60 мин созвон обсудить детали».

**Visual:** talking head only.

**Anti-pattern блока:** generic «здравствуйте» без контекста / отсутствие «почему я именно к тебе» / прыжок в Блок 2 без structure preview.

---

### Блок 2 — Кто я + что было сделано за последние пару месяцев (≈60-90 сек)

**Почему блок нужен:** calibration «не пацан с идеей — реальный output за 2 мес».

**Смысловые ключи:**
- Кто я: founder, систему строю изначально под себя, работаю с Claude Code (Antigravity environment) headfirst
- «За последние 2 месяца сделано:»
  - **Foundation Architecture LOCKED** — 11 Parts + Pillar C, 12 Constitutional rules
  - **Heptagon** — 7 strategic insights (что строю / как растёт / кто ICP / anti-extraction discipline)
  - **Charter v0** — конституция first clan
  - **Knowledge graph 722+ entries** + 100+ книг проштудировано через Claude Code (системное мышление, геймификация, смысл жизни, менеджмент)
  - **70+ гипотез монетизации** структурно расписано (с test designs)
  - **Voice pipeline + AutoResearch** — полуавтомат обработки мысли в структуру
- **Итог блока:** это не идея на салфетке — это рабочая система с output'ом

**Visual:** Antigravity в фоне (пробежка по файлам — visual доказательство); detail walkthrough в Блоке 5.

---

### Блок 3 — Что я строю (мастерская работы с информацией) (≈60-90 сек)

**Почему блок нужен:** одна sharp metaphor + bridge к МИМ Цэрэна.

**Смысловые ключи:**
- Метафора: **мастерская** — как в старом смысле, только материал = **информация** (исследования, методологии, стратегии)
- Не консалтинг / не SaaS / не AI-стартап
- 4 принципа: **ответственность, дисциплина, авантюризм, системное мышление**
- Геймификация = механика (клан 3-10 / 6 классов / квесты / 3-мес сезоны), **не суть**
- Сначала под себя → понял что может работать для тысяч других
- **Bridge:** твоя МИМ — мастерская инженеров-менеджеров с 11-летним track record. Моя — мастерская по работе с информацией. **Рядом.**

**Visual:** [Workshop Architecture mermaid (Pitch §11 M3)](outreach/jetix-mentor-partner-pitch-2026-05-12.md) — screen-share.

---

### Блок 4 — Много идей + краткий план на будущее (≈90-120 сек) ⭐ main delivery

**Почему блок нужен:** это то что Ruslan явно выделил в brief'е — «много идей структурно расписано, хочу обсудить, вот краткий план».

**Смысловые ключи:**

**4a. Прямое сообщение «идей много, они уже структурно расписаны»:**
- Это не «давай придумаем вместе»
- Это «вот 70+ направлений — давай обсудим какие резонируют и как тестить»

**4b. Краткий план на будущее (4 пункта):**

1. **Систему счастье ещё допилить** — доработать базовую систему жизнеобеспечения
2. **70+ гипотез монетизации уже есть** — surface'ировано с test designs, готово к обсуждению ([Monetization v0](decisions/JETIX-MONETIZATION-AUDIENCE-COOPERATION-METHODOLOGY-v0-2026-05-14.md) + [Wave 2](reports/monetization-research-2026-05-14/wave2/))
3. **Продвижение через сильных инженеров + лучших блогеров + продавцов** — не один, а вместе с right people
4. **Pipeline доступа к YouTube-блогерам — полуавтомат, в боевой готовности:**
   - Любого блогера можно «достать» — выйти на контакт и сделать предложение
   - Pipeline уже работает, нужна ещё **неделя доработки** → готов к запуску

**4c. Поэтому хочу обсудить на созвоне:**
- Какие из 70+ направлений резонируют с твоим опытом
- Как это стыкуется с МИМ
- Где видишь себя в этом (mentor / partner / connector / просто вовлечённый наблюдатель)

**Visual:** Antigravity screen-share — [Monetization Wave 2 folder](reports/monetization-research-2026-05-14/wave2/) видна (не объясняешь — покиваешь на масштаб).

---

### Блок 5 — Antigravity walkthrough (≈60-90 сек, screen-share)

**Почему блок нужен:** «вижу своими глазами что куча качественной информации со схемами и пояснениями». Не объясняешь детали — показываешь что это вообще есть.

**Primary visual (Tab 0):** [JETIX-DOCUMENT-MAP-2026-05-15.md](decisions/JETIX-DOCUMENT-MAP-2026-05-15.md) — карта местности с 3 mermaid (категориальная / reading ladder / граф зависимостей). Вместо открытия 6 отдельных файлов — один MAP показывает всю систему одним взглядом. Покивая на mermaid'ы.

**Сценарий walkthrough (быстро, по файлам):**
1. [JETIX-DOCUMENT-MAP](decisions/JETIX-DOCUMENT-MAP-2026-05-15.md) — Mermaid 1 (категориальная карта) + Mermaid 3 (граф зависимостей) — «вот вся карта системы одним взглядом, 9 категорий, color-coded»
2. [Workshop Architecture mermaid](outreach/jetix-mentor-partner-pitch-2026-05-12.md) — «вот архитектура мастерской, 3-layer, всё связано»
3. [Heptagon mermaid](outreach/jetix-mentor-partner-pitch-2026-05-12.md) — «7 strategic insights, каждый — отдельный lock»
4. [Foundation Architecture overview](swarm/wiki/synthesis/foundation-master-overview-technical-2026-04-29.md) — «11 Parts + Pillar C, constitutional substrate» (не расписываешь каждую Part)
5. [Charter v0 LOCKED](decisions/JETIX-FIRST-CLAN-CHARTER-2026-05-12.md) — «конституция клана, LOCKED с 12.05»
6. [Monetization Wave 2 папка](reports/monetization-research-2026-05-14/wave2/) — «H-M / H-A / H-C / H-F / H-O бакеты, test designs, industry numbers»
7. (reserve) [Knowledge graph (edges.jsonl)](wiki/graph/edges.jsonl) — «вот wiki, вот edges между концептами»

**Тон:** быстро, не задерживаясь, покивая на масштаб и качество schemes. Не «вот этот конкретный mermaid показывает...». Это не обучающее видео.

**Преимущество MAP как Tab 0:** одним документом показываешь всю архитектуру + reading tracks для receiver'а (он сразу видит «вот мой track для дальнейшего изучения»). Меньше переключений между tabs во время screen-share.

---

### Блок 6 — Конкретный ask + outro (≈45 сек)

**Почему блок нужен:** ясный next step + low-barrier exit.

**Смысловые ключи:**
- «Давай созвонимся — обсудим эти идеи»
- 3 документа пришлю отдельным сообщением (Pitch / Charter / Anton report)
- 30-60 мин на этой неделе / следующей
- «Если не интересно — спокойно скажешь, поищу дальше»
- «Спасибо, что посмотрел»

**Visual:** talking head only.

---

## §2 BRIDGE POINTS — привязка к МИМ Цэрэна

Минимум одна связка должна прозвучать:

| # | Bridge | Где прозвучит |
|---|---|---|
| 1 | «Мастерская» в названии — и МИМ, и у меня | Блок 3 |
| 2 | 11-летний track record МИМ с Левенчуком — respect и «хочу учиться у» | Блок 3 или 4 |
| 3 | Методологическая строгость (ШСМ-style) ↔ мои Constitutional rules | Блок 2 |
| 4 | «Инженеры-менеджеры» — именно таких я считаю ideal members моей мастерской | Блок 3 |
| 5 | Один из 3 ключевых mentor-candidates (с Левенчуком + Тарасовым) | Блок 4c или 6 |

---

## §3 ANTIGRAVITY SHOW PREP

Полный чек-лист с tabs / описаниями / порядком переключения — [_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md](_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md).

---

## §4 RECORDING FLOW

1. **Один дубль насквозь** — 5-7 мин от начала до конца, не останавливаясь (reality-talk style)
2. **Запнулся — продолжаешь**, не перезаписываешь (это не корпорат ролик)
3. **2-3 дубля max** — выбрать adequate, не перфектный
4. **Upload** — unlisted YouTube или TG direct
5. **TG message + видео линк** → `@TserenTserenov`

---

## §5 TELEGRAM MESSAGE (после видео)

```
Цэрэн, привет!

Записал короткое видео (5-7 мин) — важная и срочная тема. Хочу обсудить
сотрудничество и потенциально включение в First Clan.

Видео: [ссылка]

Если резонирует — пришлю 3 документа для подробного чтения и согласуем
30-60 мин созвон.

Без обязательств. Спокойно отказаться можно, если не подходит.
```

**3 docs после positive response:**
1. [Pitch Doc](outreach/jetix-mentor-partner-pitch-2026-05-12.md) (~18 мин) — [GitHub](https://github.com/Bogersebekov/jetix-os/blob/main/outreach/jetix-mentor-partner-pitch-2026-05-12.md)
2. [Charter v0](decisions/JETIX-FIRST-CLAN-CHARTER-2026-05-12.md) (~24 мин) — [GitHub](https://github.com/Bogersebekov/jetix-os/blob/main/decisions/JETIX-FIRST-CLAN-CHARTER-2026-05-12.md)
3. [Anton report](reports/anton-call-report-2026-05-11.md) (~29 мин, опционально) — [GitHub](https://github.com/Bogersebekov/jetix-os/blob/main/reports/anton-call-report-2026-05-11.md)

---

## §6 ПОСЛЕ RECORDING

1. Upload + TG message + видео → `@TserenTserenov`
2. CRM update: [crm/people/tseren-tserenov-l1.md](crm/people/tseren-tserenov-l1.md) §11 history → `2026-05-XX — Video sent. Awaits response. Charter+Pitch ready to send.`
3. `pipeline.last_touch_date: 2026-05-XX`
4. Рефлексия в Notion-странице §8 — что в итоге звучало / что поправить для видео под следующих L1 (Левенчук / Тарасов / ...)

---

## §7 СВЯЗАННЫЕ ФАЙЛЫ

- **JETIX-DOCUMENT-MAP (карта местности, Block 5 primary visual):** [decisions/JETIX-DOCUMENT-MAP-2026-05-15.md](decisions/JETIX-DOCUMENT-MAP-2026-05-15.md)
- Antigravity чек-лист (tabs prep): [_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md](_VIDEO-ANTIGRAVITY-CHECKLIST-2026-05-15.md)
- Previous version (12.05, narrower): [outreach/video-script-tseren-2026-05-12.md](outreach/video-script-tseren-2026-05-12.md)
- Pitch Doc: [outreach/jetix-mentor-partner-pitch-2026-05-12.md](outreach/jetix-mentor-partner-pitch-2026-05-12.md)
- Charter v0: [decisions/JETIX-FIRST-CLAN-CHARTER-2026-05-12.md](decisions/JETIX-FIRST-CLAN-CHARTER-2026-05-12.md)
- Anton report: [reports/anton-call-report-2026-05-11.md](reports/anton-call-report-2026-05-11.md)
- H7 People-NS: [decisions/STRATEGIC-INSIGHT-JETIX-AS-PEOPLE-NETWORK-STATE-2026-05-12.md](decisions/STRATEGIC-INSIGHT-JETIX-AS-PEOPLE-NETWORK-STATE-2026-05-12.md)
- Document pool + per-L1 sequence: [outreach/jetix-document-pool-2026-05-12.md](outreach/jetix-document-pool-2026-05-12.md)
- CRM Tseren: [crm/people/tseren-tserenov-l1.md](crm/people/tseren-tserenov-l1.md)
- Notion view (live edit): https://www.notion.so/3602496333bf815cb7e0cfc5f433046f

---

*Создано 2026-05-15. В корне репо для удобства Antigravity — все ссылки relative и кликабельные. При повторной записи под Левенчука / Тарасова / других L1 — форк файла и адаптация Блоков 3+4 под bridge points этого кандидата.*
