"""Stage 5 LLM rerank for May batch (delta to v3 baseline 2026-05-10).

Adapted from _rerun_stage5_v3.py. Operates on:
  - Input: reports/voice-pipeline-2026-05-11-may-batch/_filtered-annotated.json
    (646 NEW items = batch_2026-05-11 ∖ v3-baseline)
  - Output: reports/voice-pipeline-2026-05-11-may-batch/04-wiki-candidates-may-batch.{md,json}
  - Methodology: same as v3 — BM25 prefilter top-10 + Claude Sonnet 4.6 LLM rerank
    batch=8, thresholds A≥0.85 / B≥0.60, output suffix=may-batch.

Usage:
    python3 -m tools.wiki_integration._rerun_stage5_may_batch [--dry-run]
"""
...
OUTPUT_DIR = ROOT / "reports" / "voice-pipeline-2026-05-11-may-batch"
CKPT_PATH = OUTPUT_DIR / "_checkpoint_may_batch.json"
SUMMARY_PATH = OUTPUT_DIR / "checkpoint-summary-may-batch.md"
ANNOTATED_PATH = OUTPUT_DIR / "_filtered-annotated.json"
DISCIPLINE_LOG_PATH = OUTPUT_DIR / "_stage5_may_batch_rerun.log.md"
...
def _write_summary(state: str, stats: dict) -> None:
    body = [
        "---",
        f"title: Stage 5 May-Batch Checkpoint Summary — {state}",
        "type: audit-summary",
        f"created: {_now_utc()}",
        ...
        "---",
        "",
        f"# Stage 5 May-Batch Checkpoint Summary — {state}",
        "",
...
def _live_run() -> int:
    ...
    os.environ["STAGE5_OUTPUT_SUFFIX"] = "may-batch"
    ...
    lens = {
        "lens_name": "voice-pipeline-2026-05-11-may-batch",
    ...
    log = orch.DisciplineLog(OUTPUT_DIR / "_stage5_may_batch_rerun")
    log.path = DISCIPLINE_LOG_PATH

    _append_log(f"Stage 5 May-Batch LIVE START — {len(filtered_data.get('items', []))} items "
    ...
    _append_log(f"Stage 5 May-Batch LIVE END — state={state} elapsed={elapsed_sec}s")
...
def main(argv: list[str]) -> int:
    if "--dry-run" in argv:
        ...
        print(f"[dry-run] output suffix: may-batch")
