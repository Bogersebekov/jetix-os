...
- **scalability test:** dispatch `systems × scalability` on `agents/brigadier/strategies.md` after 50 closed cycles; expect BOSC-A-T-X trigger table for the five horizon gates (€50K / €200K / €1M / $100M / $1T per HD-01 Option C cycle-2-impl 2026-04-24) + Janus degraded-mode spec for brigadier overload (S-A excess) and brigadier deference (INT excess).
...
<!-- €50K is Ruslan's single committed absolute date (Q2 2026) per JETIX-PLAN D3.
     ...
     across the 5 expert cells + brigadier now references 5 gates. Investor-expert
     already had the €50K baseline pre-cycle-2; the 4 peer experts + brigadier
     acquired it via this cycle. S-06 convergence = 6/6 agents on 5/5 gates. -->
