---
title: Jetix Mentor/Partner Pitch — base outreach material для L1 conversations
date: 2026-05-12
type: outreach-pitch-doc
status: DRAFT-PENDING-RUSLAN-REVISION
authored_by: ai-draft (CC scribe)
prose_authored_by: ai-draft-pending-ruslan-revision
ruslan_will_rewrite: true
parent_charter: decisions/JETIX-FIRST-CLAN-CHARTER-2026-05-12.md
companion_documents:
  - reports/anton-call-report-2026-05-11.md            # narrative за 2 месяца
  - profiles/l1-first-clan/                            # 9 deep profiles
...
target_audience: "L1 9 candidates — Mentors (Левенчук / Брагинский / Тарасов) / Partner (Цэрэн) / Strategists (Федорив / Гиренко) / Investor (Хартманн) / Humanities-bridge (Дмитрий) / Aspirational anchor (Дуров)"
F: F2
G: outreach-pitch-applied-now
R: refuted_if_no_positive_response_within_30d_OR_Ruslan_rejects_framing
blast_radius: F2
launch_mode: stealth  # per Q-D3 ack 2026-05-12 — personal conversations only, NO public publication
customization: "per-person tweaks expected (base + 9 variants possible — see §8 ask-per-role)"
---
...
Я Руслан. Базируюсь в Берлине. Три слова описывают меня лучше всего: **амбициозный, дисциплинированный, ответственный**.

**Около полугода назад** я уделил ~200 часов работе со своим ментором — разбирался, что для меня важно, в чём смысл жизни, что я хочу строить дальше. После этого каждый день стал наполнен смыслом и радостью — буквально: понятный фокус, понятный результат, понятная следующая задача.

Когда я начал ценить жизнь — начал ценить свои **основные ресурсы: внимание и время**. Стал тщательно трекать, чем занимаюсь каждый час (за последний год записано уже более **8 000 часов**). Перестал потреблять любую ненужную информацию. Начал работать только над тем, что действительно важно.

Из ценности ресурсов возникла **задача управления ими**. Я начал строить под себя систему жизни — трекинг задач, привычек, времени, внимания. Параллельно глубоко изучал искусственный интеллект.

**За последние 3 месяца** результаты сошлись в одно: получилась **система по работе с информацией** — управление проектами, мониторинг самочувствия, очистка каши из головы, движение к стратегическим целям. Сначала под себя. Потом стало понятно, что та же логика может работать для тысяч других профессионалов — система готова, тестируется.

С помощью Claude Code я проштудировал **100+ книг** (системное мышление, геймификация, смысл жизни, менеджмент) и собрал описание системы + сценарии её развития. Сейчас занимаюсь самой системой и её эволюцией — это то, чем стоит делиться, поэтому пишу.

Если интересен **2-месячный детальный narrative** — `reports/anton-call-report-2026-05-11.md`. [src: anton-call-report]
...
flowchart LR
    L0["<b>L0 — NOW</b><br/>1 человек<br/><small>Founder solo (Руслан)<br/>Foundation v1.0 LOCKED 28.04<br/>Heptagon 7 insights 12.05<br/>R12 Tier 2 LOCKED 12.05</small>"]:::ladder_l0
    L1["<b>L1 — 30-60d</b><br/>5-15 человек<br/><small>First Clan<br/>9 candidates confirmed<br/>≥5 signatures triggers activation<br/>First Quest defined ≤14d</small>"]:::ladder_l1
    L2["<b>L2 — 6-12mo</b><br/>~100 человек<br/><small>Multi-clan first cohort<br/>Realm pilot launch<br/>Marathon Season 1 active</small>"]:::ladder_l2
    L3["<b>L3 — 12-24mo</b><br/>~1K человек<br/><small>Specialist federation<br/>Public Charter v1 launch<br/>STEALTH → public transition</small>"]:::ladder_l3

    L0 ==> L1 ==> L2 ==> L3

    classDef ladder_l0 fill:#e3f2fd,stroke:#1565c0,stroke-width:3px,color:#000
    classDef ladder_l1 fill:#bbdefb,stroke:#1565c0,stroke-width:3px,color:#000
    classDef ladder_l2 fill:#90caf9,stroke:#0d47a1,stroke-width:3px,color:#000
    classDef ladder_l3 fill:#64b5f6,stroke:#0d47a1,stroke-width:4px,color:#fff
```
...
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#fff8e1', 'primaryTextColor':'#000', 'primaryBorderColor':'#f57c00', 'lineColor':'#555', 'fontFamily':'Inter, system-ui, sans-serif', 'fontSize':'14px'}}}%%
flowchart TB
    H1["<b>H1 Foundation Model</b><br/><small>substrate engine</small>"]:::substrate
    H2["<b>H2 Partnership Model</b><br/><small>Manifest-style growth</small>"]:::tools
    H3["<b>H3 R&amp;D Flywheel</b><br/><small>90% reinvest</small>"]:::guard
    H4["<b>H4 Network State</b><br/><small>Balaji 5/7 NS steps</small>"]:::phone
    H5["<b>H5 Tyson Mentorship</b><br/><small>depth &gt; broadcast</small>"]:::master
    H6["<b>H6 Gamified Platform</b><br/><small>Realm operational</small>"]:::auto
    H7["<b>H7 People-Network-State</b><br/><small>synthesis substrate</small><br/>L0→L6 / 100-200y ambition"]:::storage
    ...
    classDef master fill:#fce4ec,stroke:#ad1457,stroke-width:3px,color:#000
    classDef auto fill:#f3e5f5,stroke:#6a1b9a,stroke-width:3px,color:#000
    classDef storage fill:#e0f2f1,stroke:#00695c,stroke-width:4px,color:#000
```
...
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#fff8e1', 'primaryTextColor':'#000', 'primaryBorderColor':'#f57c00', 'lineColor':'#555', 'fontFamily':'Inter, system-ui, sans-serif', 'fontSize':'14px'}}}%%
flowchart TB
    subgraph REALM ["🎮 REALM — operational layer"]
        direction TB
        ARCHETYPES["<b>6 Archetypes</b><br/><small>Hunter / Guardian / Scholar<br/>Creator / Architect / Merchant</small>"]:::auto
        QUESTS["<b>Quests + Clans</b><br/><small>real business tasks<br/>3-10 members per Clan</small>"]:::auto
        REPUTATION["<b>Reputation Ledger</b><br/><small>visible / persistent / portable</small>"]:::auto
    end

    subgraph WORKSHOP ["🏗️ WORKSHOP — substrate engine"]
        direction TB
        KB["<b>Knowledge Base</b><br/><small>wiki/ + decisions/<br/>git-tracked filesystem</small>"]:::storage
        FOUNDATION["<b>Foundation v1.0 LOCKED</b><br/><small>10 Parts + Pillar A/B/C<br/>Constitutional schemas F8</small>"]:::substrate
        METHODOLOGY["<b>Methodology Library</b><br/><small>3 mentor lineages<br/>Tyson-pattern depth</small>"]:::master
    end

    subgraph AI ["🤖 AI Substrate"]
        direction TB
        AGENTS["<b>12 Specialized Agents</b><br/><small>Manager / OPS / Sales / Brain<br/>Life / Meta departments</small>"]:::tools
    end

    subgraph BIZ ["💼 Real Business"]
        direction TB
        OUTCOMES["<b>Quest Outcomes</b><br/><small>€ / equity / reputation<br/>compound over Seasons</small>"]:::cloud
    end

    REALM ==> WORKSHOP
    WORKSHOP ==> AI
    AI ==> BIZ
    BIZ -.->|"feedback loop"| REALM
    ...
    style REALM fill:#fff8e1,stroke:#f57c00,stroke-width:4px
    style WORKSHOP fill:#fff8e1,stroke:#f57c00,stroke-width:4px
    style AI fill:#fff8e1,stroke:#f57c00,stroke-width:4px
    style BIZ fill:#fff8e1,stroke:#f57c00,stroke-width:4px
```
...
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#fff8e1', 'primaryTextColor':'#000', 'primaryBorderColor':'#f57c00', 'lineColor':'#555', 'fontFamily':'Inter, system-ui, sans-serif', 'fontSize':'14px'}}}%%
flowchart TB
    CORE(["<b>Realm</b><br/><small>L1 First Clan</small>"]):::storage

    HUNTER["<b>🏹 Hunter</b><br/><small>Sales / new business<br/>client outreach</small>"]:::partner_a
    GUARDIAN["<b>🛡️ Guardian</b><br/><small>Operations / quality<br/>processes</small>"]:::guard
    SCHOLAR["<b>📚 Scholar</b><br/><small>Research / methodology<br/>training (mentors here)</small>"]:::storage
    CREATOR["<b>🎨 Creator</b><br/><small>Content / artefacts<br/>community</small>"]:::auto
    ARCHITECT["<b>🏛️ Architect</b><br/><small>Strategy / AI<br/>system design</small>"]:::substrate
    MERCHANT["<b>💼 Merchant</b><br/><small>BizDev / partnerships<br/>capital</small>"]:::tools
...
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#fff8e1', 'primaryTextColor':'#000', 'primaryBorderColor':'#f57c00', 'lineColor':'#555', 'fontFamily':'Inter, system-ui, sans-serif', 'fontSize':'14px'}}}%%
flowchart LR
    PD["<b>Prisoner's Dilemma</b><br/><small>default state<br/>defection rational<br/>(single-round / anonymous / low-stakes)</small>"]:::guard

    subgraph MECH ["⚙️ Jetix 3 PD-breaking mechanisms"]
        direction TB
        MA["<b>M-A Repeated Games</b><br/><small>Marathon Seasons hardcoded<br/>L0→L6 multi-decade ladder<br/>(Axelrod 1984)</small>"]:::auto
        MB["<b>M-B Reputation Visibility</b><br/><small>Persona + Clan ledger<br/>public artifact track-record<br/>(Nowak 2005 / Ostrom 2009)</small>"]:::phone
        MC["<b>M-C Anti-Extraction</b><br/><small>R12 Tier 2 constitutional<br/>fork-and-leave right<br/>(Varoufakis counter)</small>"]:::substrate
    end

    COOP["<b>Cooperation Attractor</b><br/><small>dominant strategy<br/>via iterated visible play</small>"]:::master
    ...
    style MECH fill:#fff8e1,stroke:#f57c00,stroke-width:4px
```
