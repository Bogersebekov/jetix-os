...
## 9. Первая задача для роя — Phase B Self-Improvement

**Обновлено 2026-04-22 вечером, approved by Ruslan.**

Work происходит в **3-phase recursive pattern**:

### Phase A — Baseline Swarm Creation (Cloud Cowork + Claude Code на сервере сейчас)

- Claude Code читает **Tier 1 deep** + **Tier 2 required** + **Tier 3 skim**
- **Tier 4 books НЕ трогает** в Phase A (explicit — these are recursive self-improvement fuel)
- Пишет `MASTER-SYNTHESIS-HOW-TO-BUILD-BEST-SWARM` (base swarm blueprint, 20-30K слов)
- Создаёт 5 domain experts + brigadier agents (base system prompts ~1500-3000 строк each)
- Глубоко но базово — depth within scope, scope = Tier 1+2+3

### Phase B — Recursive Self-Improvement (первая real задача роя)

**Первая задача which готовый baseline swarm получает:**
- Рой сам читает **Tier 4 books** (Ousterhout / Brooks / Fowler / Ashby / Beer / Kelly / Meadows / Cagan / Grove / Laloux / Shape Up / Karpathy wiki)
- Каждый domain expert reads its domain books глубоко
- Рой **refines and deepens own system prompts + coordination protocols**
- Рой evaluates own design против fresh deeper insights
- Output: `decisions/JETIX-SWARM-V2-SELF-IMPROVED-2026-04-XX.md` + updated agents

### Phase C — Real Work

- После Phase B completion — swarm ready к real problem
- Ruslan формулирует real задачу (likely: recursive synthesis Layer 1 meta-system + Layer 2 Jetix overlay per `ROY-INFORMATION-DIET`)
- TBD exact scope

### Rationale для 3-phase pattern

- **Avoids overload:** Phase A не тонет в 393-book library, фокус на multi-agent research
- **Leverages swarm's own capability:** agents deeper-read domain books используя свою же domain lens (более efficient чем CC генерик read)
- **Compound effect:** Phase B output уже reflects matrix synergy (critic reads Ousterhout, optimizer reads Beer, etc)
- **Measurable improvement:** before/after Phase B artifacts visible в git
