---
title: Phase 2 — Beer VSM deep (Brain of the Firm + Heart of Enterprise)
date: 2026-05-19 afternoon
phase: 2
type: research-doc-vsm-deep
authored_by: brigadier-scribe
cell_dispatch: sys × cybernetics + mgmt × integrator
word_budget: 2000
status: brigadier-research-surface (R1)
parent: 00-MASTER-INDEX.md
F: F2
G: k6-beer-vsm-deep
R: refuted_if_(5_systems_not_covered OR variety_engineering_not_addressed OR Jetix_VSM_mapping_missing OR no_critique_surface)
constitutional_posture: R1 + R6 + R11 + EP-5 + IP-1 STRICT
...
flowchart TB
    ...
    S3["S3 Control<br/>(mgmt-expert + Part 7 lifecycle)"]
    S3star["S3* Audit<br/>(Provenance Officer Part 6a)"]
    S2["S2 Coordination<br/>(brigadier; hub-and-spoke)"]
    S1a["S1a Workshop sprints"]
    S1b["S1b Hackathon Tier 1-4"]
    S1c["S1c Outreach + community"]
    ALG["Algedonic channel<br/>(Part 6b Human Gate;<br/>F8/F4/F2 halt-log-alert)"]
    ENV[("External environment<br/>partners / world / signals")]
    ...
    S2 --- S1b
    S2 --- S1c
    S1a -.->|algedonic bypass| ALG
    S1b -.->|algedonic bypass| ALG
    S1c -.->|algedonic bypass| ALG
    ALG --> S5
    S4 <--> ENV
    S1a <--> ENV
    S1b <--> ENV
