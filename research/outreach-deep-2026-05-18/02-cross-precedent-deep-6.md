...
parents:
  - prompts/outreach-system-scalable-deep-research-2026-05-18.md §3 (Phase 1 spec)
  - 01-fpf-lens-scope.md (Scope B + C foregrounded)
  - decisions/strategic/JETIX-OUTREACH-SYSTEM-SCALABLE-2026-05-18.md §6 (4 baseline precedents)
cells_dispatched: eng × scalability + mgmt × integrator + investor × roi-frame
F: F2-F3
G: outreach-deep-2026-05-18-cross-precedent
R: refuted_if_(precedent factual claims contradicted by retrieved primary sources OR FPF mapping forced)
constitutional_posture: R1 + R6 + R11 + R12 + EP-5
retrieved_date_disclosure: claims grounded in pre-training corpus cutoff 2025; F3 promotion requires WebFetch verification round (deferred)
word_budget: 4200
---
...
flowchart LR
  ...
  B --> D[Pull funnel: subscribe/read]
  C --> D
  D --> E[Gumroad product + book]
  E --> F[Creator base 500M+ GMV]
  style A fill:#fff4e6
  style F fill:#d6f0d6
```
...
flowchart LR
  ...
  B --> C[Portfolio CEO]
  C --> D[Founder warm-intro]
  D --> E[Coffee chat]
  E --> F[Follow-up cohort]
  F --> G[Term sheet]
  style D fill:#fff4e6
  style G fill:#d6f0d6
```
...
flowchart LR
  ...
  C --> D[Almanack book]
  A --> E[AngelList syndicate]
  E --> F[Lead → backers → portfolio]
  D --> G[2M Twitter follow base]
  style B fill:#fff4e6
  style G fill:#d6f0d6
```
...
flowchart LR
  ...
  B --> C[Twitter 600K]
  B --> D[12 startups/12 months sprint]
  A --> E[NomadList paid community 199$]
  E --> F[Recurring substrate 3M+ ARR]
  C --> F
  style B fill:#fff4e6
  style F fill:#d6f0d6
```
...
flowchart LR
  ...
  C --> D[Regional meetups]
  D --> E[Online workshops]
  A --> F[Docs + Learning Lab]
  F --> G[Self-onboard funnel]
  A --> H[Octoverse content]
  style A fill:#fff4e6
  style G fill:#d6f0d6
```
