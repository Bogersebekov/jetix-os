---
title: Direction 13 — EVE Online 20-year virtual tribe lessons
date: 2026-05-18
type: deepening-virtual-tribe-longevity
status: brigadier-research-surface (R1)
parent: research/deepening-2026-05-18/00-DEEPENING-PLAN.md
direction_priority: 13 (medium — vision/04 + vision/08 structural pattern)
cross_refs:
  - research/adjacent-ideas-2026-05-17/06-virtual-tribes.md §1.2
  - vision/jetix-fpf-describe/04-corporation* (parallel run — NOT touched; conceptual cross-ref)
  - vision/jetix-fpf-describe/08-* (parallel run — NOT touched; conceptual cross-ref)
  - raw/voice-memos-2026-05-17-batch/text_004* (virtual tribe framing)
constitutional_posture: R1 + R6 + EP-5 disclosed
F: F4
G: eve-online-20yr-tribe-lessons
R: refuted_if_alliance_corp_structure_inapplicable_to_Jetix_OR_if_betrayal_lessons_irrelevant
...
graph TD
  ...
  EVE_E[EVE Executor Corp] -.maps to.- J_E[Jetix lead Clan / brigadier role]
  EVE_50[≥50% CEO support to executor] -.maps to.- J_50[Foundation Pillar C consent mechanism]
  EVE_BILL[2M ISK/member alliance bill] -.maps to.- J_BILL[Workshop revenue split for Foundation upkeep]
  EVE_SOV[Sovereignty + Empire Control V] -.maps to.- J_SOV[H4 Network State substrate qualifications]
  EVE_DEC[Declare Support button] -.maps to.- J_DEC[Explicit consent protocol — Foundation Part 6b]
  EVE_DISS[Lost support = alliance dissolves] -.maps to.- J_DISS[R12 fork-and-leave penalty-free]

  style EVE_C fill:#dbeafe,color:#1a202c
  style EVE_A fill:#dbeafe,color:#1a202c
  style EVE_E fill:#dbeafe,color:#1a202c
  style EVE_50 fill:#dbeafe,color:#1a202c
  style EVE_BILL fill:#dbeafe,color:#1a202c
  style EVE_SOV fill:#dbeafe,color:#1a202c
  style EVE_DEC fill:#dbeafe,color:#1a202c
  style EVE_DISS fill:#dbeafe,color:#1a202c
  style J_C fill:#bbf7d0,color:#1a202c
  style J_A fill:#bbf7d0,color:#1a202c
  style J_E fill:#bbf7d0,color:#1a202c
  style J_50 fill:#bbf7d0,color:#1a202c
  style J_BILL fill:#bbf7d0,color:#1a202c
  style J_SOV fill:#bbf7d0,color:#1a202c
  style J_DEC fill:#bbf7d0,color:#1a202c
  style J_DISS fill:#bbf7d0,color:#1a202c
```
