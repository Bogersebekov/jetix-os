...
type: research-doc
phase: 5
title: ML/AI engineer target portrait + Jetix integration (5 relationship classes)
date: 2026-05-18 evening
authored_by: brigadier-scribe
cell_dispatch: mgmt × integrator + investor × roi-frame + sales × strategy
word_budget: ~2500
fpf_layer: A.2 U.Role × Jetix-relationship classes
classes_covered: 5
acceptance_predicate: refuted_if_(<5_classes_mapped OR no_outreach_script_elements OR Karpathy_specific_omitted)
---
...
graph TB
    ...
    AP[Worker /<br/>Apprentice]
    PT[Partner /<br/>Co-founder]
    CS[Customer<br/>ML services]

    HC -->|grad to apprentice| AP
    HC -->|grad senior| HM
    HM -->|deepen commitment| AP
    HM -->|Workshop Master| AP
    AP -->|graduate| PT
    AP -->|alumni| HM
    ...
    subgraph "Specific high-priority candidates"
        ...
        SP[Sutskever]
        AN[Andrew Ng]
        L2[Russian L2<br/>Котенков/Лапань/etc]
    end

    K -.direction-09 outreach.-> PT
    SP -.long-shot.-> PT
    AN -.education adjacency.-> PT
    L2 -.outreach system note.-> HM
