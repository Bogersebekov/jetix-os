...
type: research-doc
phase: 3
title: Cohort dynamics — bloggers / engineers / investors / mentors / customers / sponsors
date: 2026-05-18 evening
authored_by: brigadier-scribe
word_budget: ~2500
cell_dispatch: mgmt × integrator + investor × roi-frame
fpf_layer: A.2 role + U.SpeechAct + A.11 commons (Phase 0 scope 4 dominant)
constitutional_posture: R1 surface + R6 + EP-5 F2-F3
acceptance_predicate: refuted_if_(<5_cohorts OR per_cohort_<5_dimensions OR no_outreach_script_element)
language: russian + english
parents:
  - 02-hackathon-variants-survey.md
  - 03-per-rhythm-mechanism-spec.md
  - research/ml-ai-engineers-2026-05-18/09-hypotheses-bank-breadth.md (H-ML-3, H-ML-10, H-ML-38, H-ML-44)
  - research/hackathon-deep-2026-05-18/05-mike-swift-mlh-deep-profile.md
  - raw/voice-memos-2026-05-17-batch/text_009@2026-05-18_evening.md ¶11 (bloggers + sponsor)
---
...
graph TD
    ...
    MENTOR[Mentors<br/>ROY swarm + RU L2] --> EVENT
    BLOGGER[Bloggers<br/>content amplification] --> EVENT
    EVENT --> ARTEFACT[Artefacts<br/>code + demos + IP]
    EVENT --> NETWORK[Network<br/>connections]
    CUSTOMER[Customers<br/>real problems] --> SPONSOR
    PARTICIPANT --> INVESTOR[Investors<br/>portfolio sourcing]
    ARTEFACT --> INVESTOR
    NETWORK --> SPONSOR
    NETWORK --> MENTOR
...
graph LR
    ...
    C --> D[100 register for event]
    D --> E[30 attend Q3 first event]
    E --> F[20 complete + retention Q4]
    F --> G[10 return for week-rhythm Q4]
```
...
graph TD
    ...
    MENTOR[Mentor]
    SPONSOR -->|Funds + theme| PARTICIPANT
    PARTICIPANT -->|Output IP retained + open-source| SPONSOR
    SPONSOR -->|Employee mentor + API credits| MENTOR
    MENTOR -->|Knowledge transfer| PARTICIPANT
    PARTICIPANT -->|Recognition + status| MENTOR
    MENTOR -->|Talent signal| SPONSOR
```
...
sequenceDiagram
    ...
    Bloggers->>Sponsor: Pre-event interview
    Jetix->>Participants: Open registration
    Participants->>Jetix: 30 attend
    Bloggers->>Audience: Live coverage
    Audience->>Jetix: Demand next event
    Sponsor->>Jetix: ROI report → next event
```
...
graph LR
    ...
    E2 --> A2[10 alumni cumulative]
    A2 --> E3[Event 3: 5 ROY + 5 ext + 10 alumni = 20]
    E3 --> A3[20 alumni cumulative]
    A3 --> EN[Event N: alumni-dominant mentor pool]
```
