...
- **Подход «бизнес как кодовая база» — 100% подтверждён** (Ruslan 2026-04-19).
  Все практики из software-индустрии (docs-as-code, git, prompt-as-code, CI/CD,
  Diátaxis, postmortems, observability) — берём. Супер-подробно, именно как
  в написании кода.
- **Детали — через agent-driven follow-up research + внутренние тесты.**
  Агенты проводят углублённый research по каждой из 8 функций, делают
  internal tests на текущем стеке (GitHub + Hetzner + Claude Code), на
  выходе дают детальную implementation-архитектуру под Jetix-specific
  ситуацию.
- Все «Ключевые вопросы» (14-дней deploy, golden datasets, eval tools,
  Diátaxis формы, postmortem templates, CI/CD, backup) — уходят в agent
  follow-up, не блокируют approval концепции.

#### 🟡 Open questions (накопитель)

- Детальный implementation plan откладывается на agent follow-up research
  (все 8 пунктов «Ключевые вопросы»)
- Связь L1 ↔ L0 research-ом (папки для human executors — это L1 decision
  тоже)

#### ✅ Финальные правки в архитектуру (v0.5 pass по L1)

- **L1 Foundation = APPROVED на концептуальном уровне.** Все 8 функций берём
  100%. Подход «бизнес = кодовая база» — фундаментальный принцип Jetix.
- Детали (minimum viable setup Q2, структура папок, tools choice, version
  policy, eval methodology, postmortem templates) — откладываются на
  **agent-driven follow-up research** + internal tests.
- Ожидаемый выход follow-up: detailed L1 implementation plan specific для
  Jetix stack (GitHub + Hetzner + Claude Code + 12+ roles + future human team).
- Статус: ✅ концептуально утверждено, требует implementation-level research
  (не блокер для остальных слоёв).
...
- **Подход Левенчука — 100% подтверждён** (Ruslan 2026-04-19). Ruslan не
  глубоко знаком с ШСМ, но **логика подходит**. L2 работает как «руль» всей
  системы — определяет как мы мыслим, как структурируем работу, как агенты
  (и люди) получают задачи.
- **Все 5 основных наработок берём:**
  - Ролевая онтология (роль ≠ исполнитель)
  - Альфы с состояниями
  - Граф создания
  - Стратегирование (метод выбора метода)
  - Мышление письмом
- **Детали — через agent-driven follow-up research + внутренние тесты.**
  Агенты делают углублённый research по применению ШСМ к AI-агентам + mega-
  corporation контексту (а не к индивидуальному знанию, как Левенчук
  изначально задумывал). Плюс практические тесты: draft FPF-lite, пример
  role-manifest, стратегирование в действии на реальной задаче.
- **Возможно делать в паре с L0 mega-research по corporate career levels.**
  L0 и L2 связаны через role-abstraction — role-manifest из L0 использует
  онтологию L2 (альфы, методы).

#### 🟡 Open questions (накопитель)

- Структура FPF-lite (3-5 стр.) — после agent follow-up research
- Формат role-manifest — связан с L0 research
- Метрика «качество framing» — нужен benchmark, agent follow-up
- Граница «selective ШСМ pick» vs deeper — решается после первой практики

#### ✅ Финальные правки в архитектуру (v0.5 pass по L2)

- **L2 Cognitive = APPROVED на концептуальном уровне.** Левенчук = руль
  системы. Все 5 примитивов зафиксированы как база.
- Детали (FPF-lite structure, role-manifest format, стратегирование
  ritual, метрики) — откладываются на **agent-driven follow-up research +
  internal tests**.
- Ожидаемый выход follow-up: working FPF-lite шаблон + example role-manifest
  + practice of стратегирование + метрика framing.
- Статус: ✅ концептуально утверждено, требует implementation-level research.
- **Связь с L0 research:** возможно объединить mega-research по corporate
  levels (L0) + deep-research по ШСМ в AI-context (L2) в один waveset,
  так как они взаимозависимы через role-abstraction.
...
- **Концепция подтверждена 100%** (Ruslan 2026-04-19): L7 = **self-governance
  механизм**. Именно это и было нужно.
- **⚠️ Расширение ресурсного scope (важная правка Ruslan'а).** В отличие
  от инвесторов (Buffett/Leonard распределяют только **капитал**), Jetix L7
  отслеживает **три ресурса одновременно:**
  - 💰 **Капитал** (деньги)
  - ⏱️ **Часы** (время)
  - 🎯 **Внимание** (attention)

  Все три ограничены, все три распределяются, все три должны быть в P&L.
- **Эволюция scope ресурсов во времени (phase-aware design):**
  - **Phase 1 (сейчас, solo):** Ruslan распоряжается **своими** деньгами,
    временем и вниманием.
  - **Phase 2 (когда появится команда):** Ruslan распоряжается своими +
    **ресурсами команды** (их часы, их внимание, их бюджеты).
  - **Phase 3 (mega-corp, C-level):** Full portfolio management большой
    компании — divisions, C-level-executives, multi-million budgets.
- **⚠️ Scale-up-first design (вторая важная правка).** Система строится
  сейчас **легонько для solo**, но **с упором на upgrade-path под
  mega-corporation**. Задача L7: при **10x росте** (капитала / часов /
  людей) — справиться **без rebuild**.
  - Легко управлять 10 проектами ✅
  - Легко управлять 10 людьми ✅
  - Легко управлять €1M+ бюджетом ✅
  - Все принципы (grading, kill, hurdle, P&L, letters) работают на любом
    масштабе, растёт только scope.

#### 🟡 Open questions (накопитель)

- Phase transitions (когда Phase 1 → 2 → 3) — какие triggers / метрики?
- Как именно scope расширяется при появлении команды (shared vs per-person
  P&L, attention budget per employee)?
- Architecture upgrade-path spec: что конкретно должно «легко
  масштабироваться» — где линия «легко» vs «нужен rebuild»?
- **Связь с L0 mega-research:** L7 scale-up требования — input для
  research по corporate career levels (как decision-rights работают
  на разных phases).

#### ✅ Финальные правки в архитектуру (v0.8 pass по L7)

- **L7 Portfolio = APPROVED 100%.** Self-governance механизм — именно
  это нужно.
- **Ресурсный scope расширен:** капитал + часы + внимание (не только
  капитал как у классического Buffett/Leonard).
- **Phase-aware design зафиксирован:** solo сегодня → team → mega-corp.
  Один principles-set, растущий scope.
- **Scale-up-first design зафиксирован:** при 10x росте (капитала /
  часов / людей / проектов) L7 должна справиться без rebuild. Это
  **обязательное требование к архитектуре слоя**, не опция.
- Статус: ✅ концептуально утверждено на все три phase, детали Phase 2/3
  — по мере приближения triggers.
...
- Ruslan 2026-04-19 сформулировал решение прямо: разделяем **полностью**,
  чтобы не было каши.
- Это **не совсем Модель A** (равные параллельные OS) и **не Модель B**
  (Life как проект в L7 Jetix) и **не Модель C** (двойной регистр). Это
  **новая Модель D — Nested hierarchy:** Life-OS выше, Jetix внутри как
  один из проектов Life-OS.

### Финальное решение — **Модель D: Nested hierarchy**
...
```
       LIFE-OS (root — моя жизнь)
         ...
         └── JETIX (один из проектов Life-OS)
                │
                └── Внутри Jetix — полноценная 7-слойная архитектура:
                    L0 / L1 / L2 / L3 / L4 / L5 / L6 / L7
                    (всё что мы только что разобрали)
```

**Ключевые принципы:**

1. **Life-OS = корневая система.** Это операционная система жизни Ruslan'а.
   Jetix — один из её проектов (самый крупный, но один из).

2. **Jetix = полноценная компания внутри.** У Jetix своя 7-слойная
   архитектура (L0-L7 которую мы проработали). Ruslan «открывает»
   Jetix и работает в нём **как полноценный пользователь/управленец
   отдельной компании**.

3. **Полное разделение ресурсов.** С этого момента:
   - **Отдельный подсчёт** часов/денег/внимания для Jetix
   - **Отдельный подсчёт** часов/денег/внимания для Life-OS
   - Jetix не конкурирует с Life за attention в одной P&L (у каждого —
     своя P&L)
   - L7 Portfolio **внутри Jetix** управляет Jetix-проектами
   - Life-OS имеет свой meta-уровень governance (отдельно)

4. **Никакой каши.** Health-метрики — в Life-OS, не в Jetix L7.
   Client retention — в Jetix L5, не в Life-OS. Business revenue — в
   Jetix L4, не в Life-OS. Это hard boundary.

5. **Физическая эволюция разделения:**
   - **Сейчас (Q2 2026):** один физический сервер, один git-репозиторий,
     но **разные папки** (например `/life-os/` и `/jetix/` на root-level
     репозитория) — логическое разделение с первого дня.
   - **Будущее (когда масштаб потребует):** **отдельный сервер для Jetix
     и отдельный сервер для Life-OS**. Отдельные git-репозитории. Полная
     инфраструктурная изоляция.
   - Заложить это в архитектуру с Day 1, чтобы при физическом разделении
     не было rebuild'а — только разнос в разные репозитории / серверы.

**Что это значит на практике для текущего Jetix OS:**

- Проект `life-os` из CLAUDE.md — **не живёт внутри Jetix**, живёт как
  отдельная корневая система Life-OS (мы создаём её параллельно).
- Проекты `engineering-thinking`, `health/`, `reflection/` —
  **переезжают в Life-OS**, не остаются в Jetix.
- Life-coach агент — живёт в Life-OS, не в Jetix ролях.
- Проекты `quick-money`, `brand`, `community`, `ai-tools`, `research`,
  `bets` (из CLAUDE.md) — реорганизуются внутри Jetix по 7 слоям
  (Шаг 4 main flow Orientation-дня).

**Связь с 8 проектами из CLAUDE.md (превью ревизии):**

| Текущий проект | Куда переезжает |
|---------------|-----------------|
| `quick-money` | Jetix L4 Delivery (primary) |
| `research` | Jetix L2 Cognitive function, снимается как проект |
| `brand` | Jetix L5 Membrane |
| `community` | Jetix L5 Membrane (Alliance + Club) |
| `ai-tools` | Jetix L3 Product + L6 future platform |
| `life-os` | **Life-OS root** (не Jetix) |
| `engineering-thinking` | **Life-OS** (личное learning) |
| `bets` | archive (заменяется L7 Portfolio discipline) |

Детальная ревизия — Шаг 4 main flow после финализации этой архитектуры.
...
- **§5 L1 и L2 — APPROVED на концептуальном уровне (v0.5, 2026-04-19).**
...
- **§5 L7 — APPROVED с расширением scope (v0.8, 2026-04-19).**
  Self-governance механизм. Ресурсы: капитал + часы + внимание (не только
  капитал как Buffett). Phase-aware: solo → team → mega-corp. Scale-up-first
  design — при 10x росте L7 справляется без rebuild.
- **§7 Company vs Life-OS — РЕШЕНО (v0.8, 2026-04-19).**
  Выбрана **Модель D: Nested hierarchy.** Life-OS = root (операционка
  жизни Ruslan'а). Jetix = один из проектов Life-OS с полноценной
...
- **Выбрана: Модель D — Nested hierarchy** (2026-04-19 Ruslan)
- **Обоснование:**
  - Life-OS = корневая система (операционка жизни Ruslan'а)
  ...
  - Никакой каши между бизнес- и личными метриками
  - Физически сейчас на одном сервере в разных папках; будущее — отдельные серверы/репозитории
...
- **L7 Portfolio — APPROVED 100%.** Self-governance механизм. Две важные
  правки Ruslan'а:
  1. Scope ресурсов расширен: капитал + часы + внимание (не только
     капитал как Buffett/Leonard).
  2. Phase-aware + scale-up-first design. При 10x росте (капитала /
     часов / людей / проектов) L7 справляется без rebuild. Это
     обязательное требование к архитектуре слоя, не опция.
  - Эволюция: Phase 1 (solo — свои ресурсы) → Phase 2 (team — +ресурсы
    команды) → Phase 3 (mega-corp — full portfolio management).
- **§7 Company vs Life-OS — РЕШЕНО.** Выбрана Модель D: Nested hierarchy.
  - Life-OS = корневая система (операционка жизни Ruslan'а)
  ...
  - Физически: сейчас один сервер, разные папки; будущее — отдельные
    серверы/репозитории. Закладываем разделение с Day 1 чтобы при scale
    не было rebuild.
  - Превью ревизии 8 проектов CLAUDE.md: `life-os` и `engineering-thinking`
    переезжают в Life-OS root; `bets` → archive; остальные
    реорганизуются по слоям Jetix.
...
- **L1 Foundation — APPROVED.** Ruslan подтвердил 100%: подход «бизнес как
  кодовая база» берём полностью, супер-подробно именно как в коде. Все 8
  функций (git, prompt-as-code, CI/CD, Diátaxis, postmortems, observability,
  risk routing, shared infra) — approved. Детали через agent follow-up
  research + internal tests на нашем stack.
- **L2 Cognitive (Левенчук) — APPROVED.** Логика Левенчука (руль системы)
  подходит. Все 5 примитивов (роль / альфа / граф создания / стратегирование /
  мышление письмом) берём. Детали (FPF-lite структура, role-manifest формат,
  метрика framing) — через agent follow-up research + тесты.
- **Идея:** объединить L0 mega-research + L2 deep-research в один waveset,
  так как они связаны через role-abstraction.
- Статус обоих: ✅ концептуально утверждено, требует implementation-level
  research, не блокер для остальных слоёв.
