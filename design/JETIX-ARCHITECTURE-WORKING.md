...
# Jetix Architecture — Working Draft v0.3
...
Сквозной. Находится внутри каждого слоя — потому что в каждом слое кто-то
должен выполнять работу. Два компонента:
- **Ruslan** (1 человек) — judgment, framing, acceptance, внешние отношения
- **Claude Code центральный** (1 session, 1M context, Opus 4.7) — выполняет
  задачи входя в 12 ролей поочерёдно через system prompt switch

Без этого слоя ничего не работает — остальные слои это артефакты, данные, метрики,
но **кто-то должен производить и принимать артефакты**.

#### ⚙️ Основные функции

1. **Исполнение задач во всех L1-L7** через 12 ролей-манифестов
2. **Judgment / framing / acceptance** — только Ruslan, не делегируется
3. **Переключение ролей** (1 Claude session → 12 масок через system prompt)
4. **Внешние коммуникации** (клиенты, партнёры, community) — только Ruslan face-to-face
5. **Обновление role-manifests** когда меняется контекст (новая модель, новый proyect)
6. **Orchestration между ролями** — manager-роль координирует остальные 11
...
- ← Все слои: поднимают high-risk approvals к Ruslan
...
12 ролей (синтезированная таблица из synthesis §3.3 и текущих 12 агентов
CLAUDE.md):
...
| delivery (implicit) | Senior | Клиентский результат | Deliverable |
...
| personal-assistant | Junior | Productivity | Задача быстрого исполнения |
| system-admin | Mid | Инфраструктура | Infra health |

*Примечание:* в текущем CLAUDE.md 12 агентов; в synthesis §3.3 предложено 12 ролей
с частично другими названиями (marketing, product, research, writer, gitops,
ops). Это **open question** — какой mapping финальный.

#### ❓ Ключевые вопросы для обсуждения

1. **Mapping 12 агентов ↔ 12 ролей synthesis** — берём CLAUDE.md имена как есть,
   или renaming по synthesis-таблице? (например, `sales-outreach` → `marketing`)?
2. **Структура role-manifest** — что именно в манифесте: 4 блока (целевая
   система + альфа + метод + acceptance) или больше (модель, tools, граф
   создания, anti-patterns, eval golden-cases)?
3. **Переключение ролей** — через `/role <name>`, через `.claude/agents/<name>/`,
   через system prompt switch, или через отдельные sessions?
4. **Ruslan как роль** — как formally он отображён? «principal» в синтезе,
   но это роль или надсистема?
5. **Партнёры** (Антон, Владислав, Родион) — роли в системе или external
   collaborators? Если роли — как их manifest выглядит?
6. **Параллелизм** — 1 Claude session может держать 12 ролей или нужны
   параллельные sessions при concurrent work?

#### 💡 Инсайты / что добавить / убрать / перефразировать

- _(пусто — заполнится во время обсуждения)_

#### 🟡 Open questions (накопитель)

- _(пусто)_

#### ✅ Финальные правки в архитектуру (после обсуждения)
