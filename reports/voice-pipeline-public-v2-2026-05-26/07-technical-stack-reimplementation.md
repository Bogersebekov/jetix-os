...
## §F.5b Детали реализации, важные для воспроизведения

Три механизма, которые методолог оценит при чтении кода:

**Идемпотентность.** Каждая стадия безопасна к повторному запуску. `transcribe.py` пропускает файлы,
у которых транскрипт уже есть. `extract.py` пропускает транскрипты с готовым `.json`. Прервался посреди
прогона — перезапустил, продолжил с места. Никаких «обработал дважды, заплатил дважды».

**Отказоустойчивость `filter.py` (partial-save).** Фильтрация — самая дорогая и долгая стадия
(opus-4-7 над всем массивом). Поэтому при `>100` единиц она идёт батчами по 50, и **каждый успешный
батч пишется на диск немедленно** (`.partials/batch_<date>_bsz50_partial_NN.json`, атомарно через
`os.replace`). Если прогон убили / упал rate-limit / kill — повторный запуск **пропускает уже
посчитанные батчи** и доделывает остаток. В конце все partials мёржатся в финальный `batch_<date>.json`
и чистятся. Флаг `--restart` игнорирует partials и считает заново. Это критично при больших corpus'ах
(1100+ единиц = 22+ батча; потерять весь прогон из-за сбоя на 20-м — больно).

**8 deliverables оркестратора.** `voice-pipeline-orchestrator.py` (reusable lens-путь) выдаёт не один
файл, а структуру (canonical §3):
```
reports/voice-pipeline-<date>/
├── 00-MASTER-INDEX.md             навигация
├── 01-per-note-breakdown.md       каждый memo → top-3 + line refs
├── 02-structured-clean.md         dedup по 12 категориям
├── 03-current-lens-actionables.md ⭐ топ-N через линзу (главный выход)
├── 04-wiki-candidates.md          match-to-existing ИЛИ propose-new (НЕ auto-merge)
├── 05-backlog-flagged.md          отложенное + CRM-черновики
├── 06-meta-analysis.md            темы / паттерны / противоречия
└── 07-discipline-log.md           провенанс + self-eval pass/fail
```
`04-wiki-candidates.md` — это материализованный выход Filter 1 (что новое, что match, что propose);
`03-current-lens-actionables.md` — выход Filter 2. Один прогон даёт обе оси фильтрации в файлах.

---

## §F.5c Сравнение с state-of-the-art (knowledge-management / second-brain)

Честное позиционирование против существующих инструментов (для методолога, который их знает). Voice-
pipeline — **не конкурент** этим продуктам, а **специфичная сборка** под голос-вход + LLM-связывание +
lens + open-substrate. Что есть у других и чего нет здесь, и наоборот:

| Инструмент | Сильная сторона | Чего НЕТ у него (что есть здесь) |
|---|---|---|
| **Obsidian** | локальный markdown-граф, плагины, backlinks | нет голос-входа из коробки; связывание ручное; нет LLM-extraction/lens |
| **Roam / Logseq** | outliner + bi-directional links + block refs | связывание ручное; нет batch голос→структура; нет двух human-gate |
| **Notion AI** | БД + AI-суммаризация в одном | closed substrate (не git, не grep); нет verbatim-anchor дисциплины; vendor lock-in |
| **Mem / Reflect** | AI-авто-связывание заметок | closed; авто-связывание без human-gate (ровно то, что здесь отключили после провала distribute.py) |
| **Otter.ai / Fireflies** | сильная транскрипция встреч | останавливаются на транскрипте; нет extraction в типизированную БД; нет графа/линзы |
| **Tiago Forte / Second Brain (метод)** | PARA + CODE как дисциплина | это метод, не инструмент; ручная organize/distill (здесь — LLM) |
| **Zettelkasten (классич.)** | атомарность + связи (проверено Луманом) | физический/ручной труд связывания; не масштабируется без машины |

**Чем эта сборка отличается принципиально:**
- **Голос как primary вход** (не клавиатура, не встречи) — закрывает разрыв скорости+места (§A).
- **Open substrate** — git + markdown + grep, не closed SaaS. Можно форкнуть, проверить, унести
  (R12 fork-friendly). Notion AI / Mem / Reflect — closed, vendor lock-in.
- **Двойная фильтрация** — Filter 1 (интеграция) + Filter 2 (lens) разнесены; у second-brain-tools
  обычно либо одно, либо другое.
- **Два обязательных human-gate** — сознательный анти-паттерн против «AI авто-связывает всё» (Mem/
  Reflect делают именно это; здесь это пробовали через `distribute.py` и **откатили** — §F.5).
- **F2/F3 формальная лестница** — verbatim anchor сохраняется явно; у большинства tools оригинал
  тонет после суммаризации.

**Где SOTA-tools лучше:** UX/полировка (это персональный инструмент одного автора, не продукт);
mobile-приложения; real-time; коллаборация многих. Voice-pipeline оптимизирован под **один сценарий**
(исследователь-изобретатель, голос, глубина, open), а не под массовый продуктовый UX.
