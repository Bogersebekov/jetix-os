---
title: arxiv-2406.07155-qian-scaling-laws
source_pdf: arxiv-2406.07155-qian-scaling-laws.pdf
...
qianc62@gmail.com
xie-zh22@mails.tsinghua.edu.cn
sms@tsinghua.edu.cn
ABSTRACT
...
leading to artifacts1 derived from their dialogues.
...
the collaborative scaling law is essential for linking performance trends with inference resources,
...
multi-:agent :collaboration :::
network (MACNET), upon which their interactive
...
E = {⟨vi, vj⟩|i, j ∈I ∧i ̸= j}
(1)
where V denotes the set of nodes indexed by the index set I, and E denotes the set of edges, with
...
ACTOR
CRITIC
Figure 3: Assign functionally bipartite agents
...
model with context-aware memory, external tools, and professional roles; ai and aij denote an actor
assigned to node vi and a critic assigned to edge vij, respectively.
2.2
...
and aij such that for every directed edge ⟨vi, vj⟩∈E, the ordering satisfies:
∀⟨vi, vj⟩∈E, I(ai) < I(aij) < I(aj)
(3)
...
and ⟲denotes an iterative process. That is, ai requests feedback, aij offers reflected suggestions
and requests further refinement, and aj provides a refined artifact. Thus, the agents associated with a
...
process preceding an artifact) remain inaccessible3 to subsequent agents (Qian et al., 2024c). This
...
consumption for the sink4 agent who experiences maximum context pressure, with and without this
mechanism, is derived as follows:
O(n)w/o = t + p + s + (2m −1)(i + s)(n(n −1)/2 + 2(n −2))
n≫1
≈Cn2 ∝n2
O(n)w/ = t + p + s + m(i + s)((n −1) + 2(n −2))
n≫1
≈
¯Cn ∝n
where
...
¯C ≡3m(i + s)
(5)
where n is the network scale (i.e., |V|), t the task length, p the profile length, i the average instruction
length, s the average artifact length, and m the maximum interaction rounds between adjacent agents.
...
• AGENTVERSE (Chen et al., 2024d) dynamically assembles and coordinates a team of expert agents
...
We adopt publicly available and logically challenging benchmarks to evaluate
...
We construct non-deterministic topologies such as trees and graphs
...
et al., 2024a; Qian et al., 2024b). Moreover, we observe that COT exhibits strong performance on
...
relatively "simple" tasks. While GPTSWARM self-organizes agents through dynamic optimization
...
adaptations be pre-made for all unforeseen complex tasks. Fortunately, MACNET bridges this gap by
...
Divergent
Convergent
Figure 6: Comparison between topologies
...
Shape Perspective
Despite the intuitive appeal of densest interactions (i.e., mesh), they do not
...
1998), characterized by a shorter average path length6 or a higher clustering coefficient7. These
...
Beyond density and shape perspectives, the inherent asymmetry in certain
...
Trend Perspective
Recall the neural scaling law, which posits that increasing neurons leads to an
...
to 26 (equating to over a thousand agents in a mesh network). As depicted in Figure 7, scaling our
...
magnitude of 24 appears to be a reasonable choice. However, considering the efficiency of sparse
...
Timing Perspective
The neural scaling law requires models with at least a billion parameters and
over 1022 training FLOPs to show emergent trends (Schaeffer et al., 2024). In contrast, collaborative
emergence in MACNET manifests at much smaller scales, with most topologies reaching performance
...
Logical Errors
Demand Errors
Non-erroneous Aspects
First-Level Aspects
Second-Level Aspects
ZeroDivisionError
Poor Test Coverage
Resource Exhaustion
PermissionError
...
aspects. Theoretically, the probability of a long-tail token t appearing at least once in n samples is:
...
In arXiv preprint arXiv:2403.02419, 2024b. URL https://arxiv.org/pdf/2403.02419.
...
Giroux,
2017.
URL https://www.pdcnet.org//collection/fshow?id=inquiryct_2012_0027_
...
In ACM Technical Symposium on Computer Science Education (SIGCSE), 2019. URL https:
...
Ruibo Liu, Ruixin Yang, Chenyan Jia, Ge Zhang, Diyi Yang, and Soroush Vosoughi. Training
...
In Product-Focused Software Process Improvement, 2009. URL https://doi.org/10.1007/
...
In Transactions on Machine Learning Research, 2022a. URL https://arxiv.org/abs/2206.
07682.
Jason Wei,
Xuezhi Wang,
Dale Schuurmans,
