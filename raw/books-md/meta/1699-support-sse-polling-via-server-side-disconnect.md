...
priority: P2
quality_grade: A
slug: 1699-support-sse-polling-via-server-side-disconnect
subcategory: meta
tags:
- meta
title: 1699 Support Sse Polling Via Server Side Disconnect
word_count: 3361
---
...
<a
href="1699-support-sse-polling-via-server-side-disconnect.html#content-area"
...
- <a
  href="1699-support-sse-polling-via-server-side-disconnect.html#abstract"
  class="break-words py-1 block hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Abstract</a>
- <a
  href="1699-support-sse-polling-via-server-side-disconnect.html#motivation"
  class="break-words py-1 block hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Motivation</a>
- <a
  href="1699-support-sse-polling-via-server-side-disconnect.html#specification"
  class="break-words py-1 block hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Specification</a>
- <a
  href="1699-support-sse-polling-via-server-side-disconnect.html#rationale"
  class="break-words py-1 block hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Rationale</a>
- <a
  href="1699-support-sse-polling-via-server-side-disconnect.html#backward-compatibility"
  class="break-words py-1 block hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Backward
  Compatibility</a>
- <a
  href="1699-support-sse-polling-via-server-side-disconnect.html#additional-information"
  class="break-words py-1 block hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Additional
  Information</a>
...
page-title="SEP-1699: Support SSE polling via server-side disconnect"
page-href="/seps/1699-support-sse-polling-via-server-side-disconnect">
...
| **Created** | 2025-10-22 |
| **Author(s)** | Jonathan Hefner (<a href="https://github.com/jonathanhefner" class="link" target="_blank"
rel="noreferrer">@jonathanhefner</a>) |
| **Sponsor** | None |
| **PR** | <a
href="https://github.com/modelcontextprotocol/modelcontextprotocol/pull/1699"
class="link" target="_blank" rel="noreferrer">#1699</a> |
...
<span data-as="p">This SEP proposes changes to the Streamable HTTP
transport in order to mitigate issues regarding long-running connections
and resumability.</span>
...
<span data-as="p">The Streamable HTTP transport spec <a
href="https://github.com/modelcontextprotocol/modelcontextprotocol/blob/04c6e1f0ea6544c7df307fb2d7c637efe34f58d3/docs/specification/draft/basic/transports.mdx?plain=1#L109-L111"
class="link" target="_blank" rel="noreferrer">does not allow</a> servers
to close a connection while computing a result. In other words, barring
client-side disconnection, servers must maintain potentially
long-running connections.</span>
...
<span data-as="p">When a server starts an SSE stream, it MUST
immediately send an SSE event consisting of an <a
href="https://html.spec.whatwg.org/multipage/server-sent-events.html#:~:text=field%20name%20is%20%22id%22"
class="link" target="_blank" rel="noreferrer"><code>id</code></a> and an
empty <a
href="https://html.spec.whatwg.org/multipage/server-sent-events.html#:~:text=field%20name%20is%20%22data%22"
class="link" target="_blank" rel="noreferrer"><code>data</code></a>
string in order to prime the client to reconnect with that event ID as
the `Last-Event-ID`.</span> <span data-as="p">Note that the SSE standard
explicitly <a
href="https://html.spec.whatwg.org/multipage/server-sent-events.html#:~:text=data%20buffer%20is%20an%20empty%20string"
class="link" target="_blank" rel="noreferrer">permits setting
<code>data</code> to an empty string</a>, and says that the appropriate
client-side handling is to record the `id` for `Last-Event-ID` but
otherwise ignore the event (i.e., not call the event handler
callback).</span> <span data-as="p">At any point after the server has
sent an event ID to the client, the server MAY disconnect at will.
Specifically, <a
href="https://github.com/modelcontextprotocol/modelcontextprotocol/blob/04c6e1f0ea6544c7df307fb2d7c637efe34f58d3/docs/specification/draft/basic/transports.mdx?plain=1#L109-L111"
class="link" target="_blank" rel="noreferrer">this part of the MCP
spec</a> will be changed from:</span>

> <span data-as="p">The server **SHOULD NOT** close the SSE stream
> before sending the JSON-RPC *response* for the received JSON-RPC
> *request*</span>

<span data-as="p">To:</span>

> <span data-as="p">The server **MAY** close the connection before
> sending the JSON-RPC *response* if it has sent an SSE event with an
> event ID to the client</span>

<span data-as="p">If a server disconnects, the client will interpret the
disconnection the same as a network failure, and will attempt to
reconnect. In order to prevent clients from reconnecting / polling
excessively, the server SHOULD send an SSE event with a <a
href="https://html.spec.whatwg.org/multipage/server-sent-events.html#:~:text=field%20name%20is%20%22retry%22"
class="link" target="_blank" rel="noreferrer"><code>retry</code></a>
field indicating how long the client should wait before reconnecting.
Clients MUST respect the `retry` field.</span>
...
<span data-as="p">Servers may disconnect at will, avoiding long-running
connections. Sending a `retry` field will prevent the client from
hammering the server with inappropriate reconnection attempts.</span>
...
- **Old Client + New Server**: Client should interpret an at-will
  disconnect the same as a network failure. `retry` field is part of the
  SSE standard. No backward incompatibility if client already implements
  proper SSE resuming logic.
...
<span class="cursor-pointer">Additional Information</span>

<span data-as="p">This SEP supersedes (in part) <a
href="https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1335"
class="link" target="_blank" rel="noreferrer">SEP-1335</a>.</span>
...
<a href="1686-tasks.html" class="flex items-center space-x-3 group"><img
