...
priority: P2
quality_grade: A
slug: 1303-input-validation-errors-as-tool-execution-errors
subcategory: meta
tags:
- meta
title: 1303 Input Validation Errors As Tool Execution Errors
word_count: 4597
---
...
<a
href="1303-input-validation-errors-as-tool-execution-errors.html#content-area"
...
- <a
  ...
  style="padding-left:1rem">Problem Statement</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#benefits-of-this-proposal"
  class="group flex items-start break-words py-1 whitespace-pre-wrap text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  style="padding-left:1rem">Benefits of This Proposal</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#specification"
  class="break-words py-1 block font-medium hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Specification</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#current-behavior"
  class="group flex items-start break-words py-1 whitespace-pre-wrap text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  style="padding-left:1rem">Current Behavior</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#proposed-change"
  class="group flex items-start break-words py-1 whitespace-pre-wrap text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  style="padding-left:1rem">Proposed Change</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#specification-text-changes"
  class="group flex items-start break-words py-1 whitespace-pre-wrap text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  style="padding-left:1rem">Specification Text Changes</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#implementation"
  class="break-words py-1 block font-medium hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Implementation</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#before-protocol-error"
  class="group flex items-start break-words py-1 whitespace-pre-wrap text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  style="padding-left:1rem">Before (Protocol Error)</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#after-tool-execution-error"
  class="group flex items-start break-words py-1 whitespace-pre-wrap text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  style="padding-left:1rem">After (Tool Execution Error)</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#backwards-compatibility"
  class="break-words py-1 block font-medium hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">Backwards
  Compatibility</a>
- <a
  href="1303-input-validation-errors-as-tool-execution-errors.html#references"
...
page-title="SEP-1303: Input Validation Errors as Tool Execution Errors"
page-href="/seps/1303-input-validation-errors-as-tool-execution-errors">
...
| **Created** | 2025-08-05 |
| **Author(s)** | <a href="https://github.com/fredericbarthelet" class="link"
target="_blank" rel="noreferrer">@fredericbarthelet</a> |
| **Sponsor** | None |
| **PR** | <a
href="https://github.com/modelcontextprotocol/modelcontextprotocol/pull/1303"
class="link" target="_blank" rel="noreferrer">#1303</a> |
...
<span data-as="p">This SEP proposes treating tools input validation
errors as Tool Execution Errors rather than Protocol Errors. This change
would enable language models to receive validation error feedback in
their context window, allowing them to self-correct and successfully
complete tasks without human intervention, significantly improving task
...
<span data-as="p">Language models can learn from tool input validation
error messages and retry a tools/call with corrected parameters
accordingly, but only if they receive the error feedback in their
context window. Protocol Errors are catch at the application level by
the MCP Client. Only Tool Execution Errors are forwarded back to the
model as JSON-RPC responses. With the current specifications, models
cannot see these error messages and thus cannot self-correct, leading to
repeated failures and poor user experiences.</span>
...
<span class="cursor-pointer">Problem Statement</span>

<span data-as="p">Consider a flight booking tool that validates
departure dates using the following `zod` validation schema:</span>
...
numberoflines="15" language="typescript">
...
<span data-as="p">Tool expected input JSON schema can only describe the
regex statement. The actual programmatic check that the date is in the
past cannot be expressed here as JSON schema. Even when a model provides
a syntactically correct date that passes JSON schema validation, there
is no guarantee it will be in the future. When a validation error is
raised and returned as a Protocol Error:</span>

1.  The model doesn’t receive the error message explaining why the date
    was rejected
2.  The model repeats the same mistake multiple times (e.g., Cursor
    typically consistently sends dates in 2024 when the user only
    specify day and month or relative date and repeats the same
    tools/call request 3 times without getting any information as to why
    the tools call fails)
3.  The task fails despite the model being capable of correcting itself
    if given proper feedback
...
<span class="cursor-pointer">Benefits of This Proposal</span>

1.  **Higher Task Completion Rates**: Models can self-correct validation
    errors without human intervention
2.  **Better User Experience**: Reduced failures and faster task
    completion
3.  **Leverages Model Capabilities**: Modern LLMs excel at understanding
    and responding to error messages
4.  **Reduced API Calls**: Fewer retry attempts as models correct
    themselves on the first error
...
<span class="cursor-pointer">Current Behavior</span>

<span data-as="p">The <a
href="https://modelcontextprotocol.io/specification/2025-06-18/server/tools#error-handling"
class="link" target="_blank" rel="noreferrer">tool errors
specification</a> currently provides ambiguous guidance:</span>

- “Invalid arguments” should be treated as Protocol Error
- “Invalid input data” should be treated as Tool Execution Error

<span data-as="p">This ambiguity leads to inconsistent implementations
where valuable error feedback is lost.</span>
...
<span class="cursor-pointer">Proposed Change</span>

<span data-as="p">Clarify the specification with the following
changes:</span>

1.  Removes the “invalid argument” category from **Protocol Errors**.
2.  **Tool Execution Errors** should be used for all tool argument
    validation failures (merging `invalid argument` and
    `invalid input data` under a new `input validation errors` category)
...
<span class="cursor-pointer">Specification Text Changes</span>

<span data-as="p">Update the error handling section to include:</span>

<div class="code-block mt-5 mb-8 not-prose rounded-2xl relative group min-w-0 print:print-color-exact text-gray-950 dark:text-gray-50 codeblock-light border border-gray-950/10 dark:border-white/10 dark:twoslash-dark bg-transparent dark:bg-transparent"
numberoflines="14" language="text">
...
<span class="cursor-pointer">Implementation</span>
...
<span class="cursor-pointer">Before (Protocol Error)</span>

<div class="code-block mt-5 mb-8 not-prose rounded-2xl relative group min-w-0 print:print-color-exact text-gray-950 dark:text-gray-50 codeblock-light border border-gray-950/10 dark:border-white/10 dark:twoslash-dark bg-transparent dark:bg-transparent"
numberoflines="23" language="typescript">
...
<span class="cursor-pointer">After (Tool Execution Error)</span>

<div class="code-block mt-5 mb-8 not-prose rounded-2xl relative group min-w-0 print:print-color-exact text-gray-950 dark:text-gray-50 codeblock-light border border-gray-950/10 dark:border-white/10 dark:twoslash-dark bg-transparent dark:bg-transparent"
numberoflines="36" language="typescript">
...
<span class="cursor-pointer">Backwards Compatibility</span>

<span data-as="p">This change is backwards compatible as it:</span>
...
<span data-as="p">Servers implementing the clarified behavior will
provide better model self-recovery while continuing to work with all
...
- <a
  href="https://modelcontextprotocol.io/specification/2025-06-18/server/tools#error-handling"
  class="link" target="_blank" rel="noreferrer">MCP Tools Error Handling
  Specification</a>
- <a
  href="https://dev.to/alpic/better-mcp-toolscall-error-responses-help-your-ai-recover-gracefully-15c7"
  class="link" target="_blank" rel="noreferrer">Better MCP tools/call
  Error Responses: Help Your AI Recover Gracefully</a>
- Related Issue: <a
  href="https://github.com/modelcontextprotocol/typescript-sdk/pull/824"
  class="link" target="_blank"
  rel="noreferrer">https://github.com/modelcontextprotocol/typescript-sdk/pull/824</a>
...
<a href="1302-formalize-working-groups-and-interest-groups-in-mc.html"
...
class="group-hover:text-gray-900 dark:group-hover:text-white">SEP-1302:
Formalize Working Groups and Interest G…</span></a><a href="1319-decouple-request-payload-from-rpc-methods-definiti.html"
class="flex items-center ml-auto space-x-3 group"><span
class="group-hover:text-gray-900 dark:group-hover:text-white">SEP-1319:
Decouple Request Payload from RPC Metho…</span><img
