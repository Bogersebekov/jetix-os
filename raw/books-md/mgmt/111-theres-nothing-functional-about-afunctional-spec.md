---
acquired_date: '2026-04-22'
converted_date: '2026-04-22'
converted_via: docling
expert: mgmt-expert
priority: P1
quality_grade: A
slug: 111-theres-nothing-functional-about-afunctional-spec
subcategory: mgmt
tags:
- mgmt
title: 111 Theres Nothing Functional About Afunctional Spec
word_count: 1642
---

<div id="main" class="wb" role="main">

**Heads up!** This page uses features your browser doesn’t support. Try
a modern browser like
<a href="https://www.mozilla.org/en-US/firefox/new/" target="_blank"
rel="noopener noreferrer">Firefox</a> or
<a href="https://www.google.com/chrome/" target="_blank"
rel="noopener noreferrer">Chrome</a> for the best experience.

<div class="intro">

<div class="intro__content intro__content--sticky">

<img
src="data:image/svg+xml;base64,"
class="icon" /> Getting Real

Chapter 59:

# [There’s Nothing Functional about a Functional Spec](11.1-theres-nothing-functional-about-afunctional-spec.html)

[Next: Don’t Do Dead Documents](11.2-dont-do-dead-documents.html)

</div>

</div>

<div class="content" controller="bookmark anchors tweet"
data-target="sidebar.content"
data-action="click-&gt;sidebar#close mouseup-&gt;tweet#update input-&gt;tweet#update keydown-&gt;tweet#update scroll@window-&gt;tweet#update"
bookmark-id="/gettingreal">

<img
src="data:image/svg+xml;base64,"
class="icon icon--link" /> <img
src="data:image/svg+xml;base64,"
class="icon icon--twitter" />

## Don’t write a functional specifications document

These blueprint docs usually wind up having almost nothing to do with
the finished product. Here’s why:

## Functional specs are fantasies

They don’t reflect reality. An app is not real until builders are
building it, designers are designing it, and people are using it.
Functional specs are just words on paper.

## Functional specs are about appeasement

They’re about making everyone feel involved and happy which, while warm
and fuzzy, isn’t all that helpful. They’re never about making tough
choices and exposing costs, things that need to happen to build a great
app.

## Functional specs only lead to an illusion of agreement

A bunch of people agreeing on paragraphs of text isn’t a true agreement.
Everyone may be reading the same thing but they’re thinking something
different. This inevitably comes out later on: “Wait, that’s not what I
had in mind.” “Huh? That’s not how we described it.” “Yes it was and we
all agreed on it — you even signed off on it.” You know the drill.

## Functional specs force you to make the most important decisions when you have the least information

You know the least about something when you begin to build it. The more
you build it, the more you use it, the more you know it. That’s when you
should be making decisions — when you have more information, not less.

<div class="page-break">

</div>

## Functional specs lead to feature overload

There’s no pushback during spec phase. There’s no cost to writing
something down and adding another bullet point. You can please someone
who’s a pain by adding their pet feature. And then you wind up designing
to those bullet points, not to humans. And that’s how you wind up with
an overloaded site that’s got 30 tabs across the top of the screen.

## Functional specs don’t let you evolve, change,and reassess

A feature is signed off and agreed on. Even if you realize during
development that it’s a bad idea, you’re stuck with it. Specs don’t deal
with the reality that once you start building something, everything
changes.

So what should you do in place of a spec? Go with a briefer alternative
that moves you toward something real. Write a one page story about what
the app needs to do. Use plain language and make it quick. If it takes
more than a page to explain it, then it’s too complex. This process
shouldn’t take more than one day.

Then begin building the interface — the interface will be the
alternative to the functional spec. Draw some quick and simple paper
sketches. Then start coding it into html. Unlike paragraphs of text that
are open to alternate interpretations, interface designs are common
ground that everyone can agree on.

Confusion disappears when everyone starts using the same screens. Build
an interface everyone can start looking at, using, clicking through, and
“feeling” before you start worrying about back-end code. Get yourself in
front of the customer experience as much as possible.

Forget about locked-in specs. They force you to make big, key decisions
too early in the process. Bypass the spec phase and you’ll keep change
cheap and stay flexible.

### Useless Specs

> A “spec” is close to useless. I have never seen a spec that was both
> big enough to be useful and accurate.
>
> And I have seen lots of total crap work that was based on specs. It’s
> the single worst way to write software, because it by definition means
> that the software was written to match theory, not reality.

—Linus Torvalds, creator of [Linux](http://www.linux.org/) (from:
[Linux: Linus On
Specifications](https://web.archive.org/web/20060821225841/http://kerneltrap.org/node/5725))

### Fight the blockers

> I found the people insisting on extensive requirements documents
> before starting any design were really ‘blockers’ just trying to slow
> the process down (and usually people with nothing to contribute on
> design or innovative thinking).
>
> All our best work was done with a few concepts in our heads about
> improving a site, doing a quick prototype (static), changing the
> design a bit and then building a live prototype with real data. After
> kicking the tires on this prototype, we usually had a real project in
> motion and good result.

—Mark Gallagher, corporate intranet developer (from [Signal vs.
Noise](https://signalvnoise.com/archives2/an_interview_about_basecamp_backpack_getting_real_and_more_at_tompeterscom.php))

<a href="11.2-dont-do-dead-documents.html" class="button">Don’t Do Dead
Documents →</a>

We made <a href="https://basecamp.com" target="_blank">Basecamp</a>
using the principles in this book. It combines all the tools teams need
to get work done in a single, streamlined package. With Basecamp,
everyone knows what to do, where things stand, and where to find things
they need.

*Copyright ©1999-2025 37signals LLC. All rights reserved.*

</div>

<img
src="data:image/svg+xml;base64,"
class="icon" />

<div class="book-info">

[*Back to Basecamp.com*](https://basecamp.com/)

# [Getting Real](../gettingreal.html)

The smarter, faster, easier way to build a successful web application

by Basecamp

</div>

<div class="toc">

<div class="toc__part">

## Introduction

- Chapter 1

  ### [What is Getting Real](01.1-what-is-getting-real.html)

- Chapter 2

  ### [About Basecamp](01.2-about-basecamp.html)

- Chapter 3

  ### [Caveats, disclaimers, and other preemptive strikes](01.3-caveats-disclaimers-and-other-preemptive-strikes.html)

</div>

<div class="toc__part">

## The Starting Line

- Chapter 4

  ### [Build Less](02.1-build-less.html)

- Chapter 5

  ### [What’s Your Problem?](02.2-whats-your-problem.html)

- Chapter 6

  ### [Fund Yourself](02.3-fund-yourself.html)

- Chapter 7

  ### [Fix Time and Budget, Flex Scope](02.4-fix-time-and-budget-flex-scope.html)

- Chapter 8

  ### [Have an Enemy](02.5-have-an-enemy.html)

- Chapter 9

  ### [It Shouldn’t be a Chore](02.6-it-shouldnt-be-a-chore.html)

</div>

<div class="toc__part">

## Stay Lean

- Chapter 10

  ### [Less Mass](03.1-less-mass.html)

- Chapter 11

  ### [Lower Your Cost of Change](03.2-lower-your-cost-of-change.html)

- Chapter 12

  ### [The Three Musketeers](03.3-the-three-musketeers.html)

- Chapter 13

  ### [Embrace Constraints](03.4-embrace-constraints.html)

- Chapter 14

  ### [Be Yourself](03.5-be-yourself.html)

</div>

<div class="toc__part">

## Priorities

- Chapter 15

  ### [What’s the Big Idea?](04.1-whats-the-big-idea.html)

- Chapter 16

  ### [Ignore Details Early On](04.2-ignore-details-early-on.html)

- Chapter 17

  ### [It’s a Problem When It’s a Problem](04.3-its-a-problem-when-its-a-problem.html)

- Chapter 18

  ### [Hire the Right Customers](04.4-hire-the-right-customers.html)

- Chapter 19

  ### [Scale Later](04.5-scale-later.html)

- Chapter 20

  ### [Make Opinionated Software](04.6-make-opinionated-software.html)

</div>

<div class="toc__part">

## Feature Selection

- Chapter 21

  ### [Half, Not Half-Assed](05.1-half-not-half-assed.html)

- Chapter 22

  ### [It Just Doesn’t Matter](05.2-it-just-doesnt-matter.html)

- Chapter 23

  ### [Start With No](05.3-start-with-no.html)

- Chapter 24

  ### [Hidden Costs](05.4-hidden-costs.html)

- Chapter 25

  ### [Can You Handle It?](05.5-can-you-handle-it.html)

- Chapter 26

  ### [Human Solutions](05.6-human-solutions.html)

- Chapter 27

  ### [Forget Feature Requests](05.7-forget-feature-requests.html)

- Chapter 28

  ### [Hold the Mayo](05.8-hold-the-mayo.html)

</div>

<div class="toc__part">

## Process

- Chapter 29

  ### [Race to Running Software](06.1-race-to-running-software.html)

- Chapter 30

  ### [Rinse and Repeat](06.2-rinse-and-repeat.html)

- Chapter 31

  ### [From Idea to Implementation](06.3-from-idea-to-implementation.html)

- Chapter 32

  ### [Avoid Preferences](06.4-avoid-preferences.html)

- Chapter 33

  ### [“Done!”](06.5-done.html)

- Chapter 34

  ### [Test in the Wild](06.6-test-in-the-wild.html)

- Chapter 35

  ### [Shrink Your Time](06.7-shrink-your-time.html)

</div>

<div class="toc__part">

## The Organization

- Chapter 36

  ### [Unity](07.1-unity.html)

- Chapter 37

  ### [Alone Time](07.2-alone-time.html)

- Chapter 38

  ### [Meetings Are Toxic](07.3-meetings-are-toxic.html)

- Chapter 39

  ### [Seek and Celebrate Small Victories](07.4-seek-and-celebrate-small-victories.html)

</div>

<div class="toc__part">

## Staffing

- Chapter 40

  ### [Hire Less and Hire Later](08.1-hire-less-and-hire-later.html)

- Chapter 41

  ### [Kick the Tires](08.2-kick-the-tires.html)

- Chapter 42

  ### [Actions, Not Words](08.3-actions-not-words.html)

- Chapter 43

  ### [Get Well Rounded Individuals](08.4-get-well-rounded-individuals.html)

- Chapter 44

  ### [You Can’t Fake Enthusiasm](08.5-you-cant-fake-enthusiasm.html)

- Chapter 45

  ### [Wordsmiths](08.6-wordsmiths.html)

</div>

<div class="toc__part">

## Interface Design

- Chapter 46

  ### [Interface First](09.1-interface-first.html)

- Chapter 47

  ### [Epicenter Design](09.2-epicenter-design.html)

- Chapter 48

  ### [Three State Solution](09.3-three-state-solution.html)

- Chapter 49

  ### [The Blank Slate](09.4-the-blank-slate.html)

- Chapter 50

  ### [Get Defensive](09.5-get-defensive.html)

- Chapter 51

  ### [Context Over Consistency](09.6-context-over-consistency.html)

- Chapter 52

  ### [Copywriting is Interface Design](09.7-copywriting-is-interface-design.html)

- Chapter 53

  ### [One Interface](09.8-one-interface.html)

</div>

<div class="toc__part">

## Code

- Chapter 54

  ### [Less Software](10.1-less-software.html)

- Chapter 55

  ### [Optimize for Happiness](10.2-optimize-for-happiness.html)

- Chapter 56

  ### [Code Speaks](10.3-code-speaks.html)

- Chapter 57

  ### [Manage Debt](10.4-manage-debt.html)

- Chapter 58

  ### [Open Doors](10.5-open-doors.html)

</div>

<div class="toc__part">

## Words

- Chapter 59

  ### [There’s Nothing Functional about a Functional Spec](11.1-theres-nothing-functional-about-afunctional-spec.html)

- Chapter 60

  ### [Don’t Do Dead Documents](11.2-dont-do-dead-documents.html)

- Chapter 61

  ### [Tell Me a Quick Story](11.3-tell-me-a-quick-story.html)

- Chapter 62

  ### [Use Real Words](11.4-use-real-words.html)

- Chapter 63

  ### [Personify Your Product](11.5-personify-your-product.html)

</div>

<div class="toc__part">

## Pricing and Signup

- Chapter 64

  ### [Free Samples](12.1-free-samples.html)

- Chapter 65

  ### [Easy On, Easy Off](12.2-easy-on-easy-off.html)

- Chapter 66

  ### [Silly Rabbit, Tricks are for Kids](12.3-silly-rabbit-tricks-are-for-kids.html)

- Chapter 67

  ### [A Softer bullet](12.4-a-softer-bullet.html)

</div>

<div class="toc__part">

## Promotion

- Chapter 68

  ### [Hollywood Launch](13.1-hollywood-launch.html)

- Chapter 69

  ### [A Powerful Promo Site](13.2-a-powerful-promo-site.html)

- Chapter 70

  ### [Ride the Blog Wave](13.3-ride-the-blog-wave.html)

- Chapter 71

  ### [Solicit Early](13.4-solicit-early.html)

- Chapter 72

  ### [Promote Through Education](13.5-promote-through-education.html)

- Chapter 73

  ### [Feature Food](13.6-feature-food.html)

- Chapter 74

  ### [Track Your Logs](13.7-track-your-logs.html)

- Chapter 75

  ### [Inline Upsell](13.8-inline-upsell.html)

- Chapter 76

  ### [Name Hook](13.9-name-hook.html)

</div>

<div class="toc__part">

## Support

- Chapter 77

  ### [Feel The Pain](14.1-feel-the-pain.html)

- Chapter 78

  ### [Zero Training](14.2-zero-training.html)

- Chapter 79

  ### [Answer Quick](14.3-answer-quick.html)

- Chapter 80

  ### [Tough Love](14.4-tough-love.html)

- Chapter 81

  ### [In Fine Forum](14.5-in-fine-forum.html)

- Chapter 82

  ### [Publicize Your Screwups](14.6-publicize-your-screwups.html)

</div>

<div class="toc__part">

## Post-Launch

- Chapter 83

  ### [One Month Tuneup](15.1-one-month-tuneup.html)

- Chapter 84

  ### [Keep the Posts Coming](15.2-keep-the-posts-coming.html)

- Chapter 85

  ### [Better, Not Beta](15.3-better-not-beta.html)

- Chapter 86

  ### [All Bugs Are Not Created Equal](15.4-all-bugs-are-not-created-equal.html)

- Chapter 87

  ### [Ride Out the Storm](15.5-ride-out-the-storm.html)

- Chapter 88

  ### [Keep Up With the Joneses](15.6-keep-up-with-the-joneses.html)

- Chapter 89

  ### [Beware the Bloat Monster](15.7-beware-the-bloat-monster.html)

- Chapter 90

  ### [Go With The Flow](15.8-go-with-the-flow.html)

</div>

<div class="toc__part">

## Conclusion

- Chapter 91

  ### [Start Your Engines](16.1-start-your-engines.html)

</div>

</div>

</div>