...
status: DRAFT — Ruslan R1 prose pass required для finalisation; server CC deep version будет через prompts/navigation-guide-deep-2026-05-23.md
sibling_prompt: prompts/navigation-guide-deep-2026-05-23.md
sibling_explain: prompts/explanations/_EXPLAIN-navigation-guide-deep-2026-05-23.md
---
...
graph TD
    Repo[jetix-os repo]
    Repo --> Entry[Entry points]
    Repo --> Strategic[Strategic substrate]
    Repo --> Foundation[Foundation v1.0]
    Repo --> Wiki[Wiki concepts]
    Repo --> Operational[Operational]

    Entry --> E1[Video walkthrough]
    Entry --> E2[Partner Offering Human-Lang]
    Entry --> E3[Этот Navigation Guide]

    Strategic --> S1[Method V2 ⭐⭐⭐]
    Strategic --> S2[Strategic Plan ⭐⭐]
    Strategic --> S3[Economic Model V10 ⭐⭐]
    Strategic --> S4[AI Market PLAN Stage 1]

    Foundation --> F1[11 Parts F5 LOCKED]
    Foundation --> F2[Pillar A Strategic Direction]
    Foundation --> F3[Pillar C Principles + R12 LOCK]

    Wiki --> W1[13 Tier A concepts]
    Wiki --> W2[Method-method one-liner O-107]
    Wiki --> W3[8-component meta-method O-121]
    Wiki --> W4[External-system cybernetic O-128]

    Operational --> O1[Daily Logs]
    Operational --> O2[Reports voice batches]
    Operational --> O3[Research DR-26+]
    Operational --> O4[Prompts server CC]

    style E1 fill:#ffe0a0
    style E2 fill:#ffe0a0
    style E3 fill:#ffe0a0
...
%%{init: {'theme':'base','themeVariables':{'primaryTextColor':'#000','textColor':'#000','lineColor':'#333','primaryBorderColor':'#333','primaryColor':'#fafafa'}}}%%
graph LR
    A[Всё = информация<br/>+ методы обработки]
    A --> B[Интеллект<br/>= переработчик информации<br/>+ применитель методов]
    B --> C[Качество жизни / развития<br/>= качество методов<br/>× качество их выбора]
    C --> D[Метод выбора методов<br/>= ключевой leverage]
    D --> E[AGI minimal formula<br/>system w/ info+methods accumulated<br/>> other system/task]
    E --> F[Jetix-lever:<br/>curated info-corpus<br/>+ method-arsenal<br/>+ meta-method]
...
%%{init: {'theme':'base','themeVariables':{'primaryTextColor':'#000','textColor':'#000','lineColor':'#333','primaryBorderColor':'#333','primaryColor':'#fafafa'}}}%%
graph TB
    Stage1[Этап 1<br/>Платформа на Ethereum L2<br/>R12 + V10 Hybrid + 75/25 + Mondragón 5:1]
    Stage2[Этап 2<br/>Собрать влиятельных людей<br/>Wave 1 → Layer 1 → Layer 2 → Layer 3]
    Stage3[Этап 3<br/>Платформа управления проектами<br/>+ cohort communication + Workshop]
    Stage4[Этап 4<br/>Universal advanced people<br/>Workshop + Wiki v2 + ROY swarm]
    Stage5[Этап 5<br/>Healthy society development<br/>library + governance + adequate trained people]
...
gantt
    ...
    section MVP Sprint
    Layer 1 Foundation cohort              :L1, 2026-06-01, 7d
    Layer 2 Platform Infra                 :L2, after L1, 7d
    Layer 3 Features                       :L3, after L2, 7d
    Polish + Launch ready                   :polish, after L3, 9d
    section Mass distribution
    Public alpha launch                    :crit, mass, 2026-07-01, 31d
    Workshop intake at scale               :ws, 2026-07-01, 31d
```
...
%%{init: {'theme':'base','themeVariables':{'primaryTextColor':'#000','textColor':'#000','lineColor':'#333','primaryBorderColor':'#333','primaryColor':'#fafafa'}}}%%
graph LR
    subgraph Library[Jetix = библиотека + система]
        L1[Только полезная информация<br/>для развития / бизнеса / общества]
        L2[Только полезные методы<br/>для тех же целей]
        L3[Доступ через cohort membership<br/>L4-L7 tiers]
    end

    Library --> Govern[Self-governance<br/>через R12 + Pillar C + Charter]
    Library --> Train[Workshop intake<br/>mastery formula hands-on]

    Train --> People[Adequate trained люди<br/>с meta-method capacity]
    People --> Manage[Управление сложными системами<br/>своими + societal-level]
    Manage --> Healthy[Healthy society development]

    Govern -.constitutional protection.-> Library
    Healthy -.feedback.-> Library

    style Library fill:#fff8d5
    style People fill:#d6f0d6
    style Healthy fill:#d6e0f0
```
...
sequenceDiagram
    ...
    Ruslan->>Recipient: Wave 1 send (video + 3 docs + repo link)
    Recipient->>Substrate: Read (15 min - 2h)
    Recipient->>Ruslan: Initial feedback
    Ruslan->>Recipient: Call booking (30-60 min)
    Recipient->>Ruslan: Co-strategising round 1
    Ruslan->>Substrate: Integrate feedback (R1 prose pass)
    Substrate->>Recipient: Updated substrate share
    Recipient->>Ruslan: Co-strategising round 2-N
    Ruslan->>Recipient: Layer 1 Founding offer (if alignment)
    Recipient->>Ruslan: Accept / Decline / Defer
```
