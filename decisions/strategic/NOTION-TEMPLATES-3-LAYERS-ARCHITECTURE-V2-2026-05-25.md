...
### Минимальный состав (10 баз + Strategic + Reviews)

| # | База | Статус | Назначение в одну строку |
|---|---|---|---|
| 1 | 📅 **Daily Log** | core | день = одна запись: цель, что сделал, энергия, привычки, связи |
...
orientation) · Linked Project / People / Hypothesis / Ideas (Relations) · **Habits done today**
(Relation → Habits — какие привычки выполнил сегодня; день сам помнит, без отдельной записи на
привычку).
...
### §2.3b Hypotheses / Life Pulse / Habits / Финансы
...
- **🔁 Habits** (core-lite, новое — полезно почти всем) — список регулярных привычек: Name · Cadence
  (daily / weekly / Nx-week) · Target (словами) · Area (health / mind / work / relationships) ·
  Active · Days done (rollup ← Daily Log) · Notes. **Трекинг без задроченности:** факт «сделал
  сегодня» линкуешь в Daily Log (поле «Habits done today»), а Habits через rollup сам считает,
  сколько раз привычка отмечена за период — НЕ заводишь отдельную запись на каждую привычку каждый
  день. Где Life Pulse замеряет состояние, Habits фиксирует действие. Streak/хитмап + отдельная
  Habit-Log база — в sidebar (осторожно с геймификацией: streak это зеркало, а не кнут — R12 к себе).
  **Приватность:** как Life Pulse — наверх не уходит.
...
страниц-документов**: три про направление (куда иду) и две про смысл/мудрость (что хочу помнить).
...
4. **🧠 Философский лист (страница — что хочу помнить).** *Новое по запросу Руслана.* Отдельная
   страница для **всего, что хочешь держать перед глазами**: жизненные мудрости, наставления (себе
   и от других), принципы, правила жизни, выводы. Отличие от Vision: Vision = «куда я иду», а
   философский лист = «как я хочу жить и думать по дороге». Vision меняется со сменой направления;
   философский лист растёт всю жизнь (append-only). Перечитываешь в месячном/квартальном ревью.
   Глубоко личный — CC сюда не пишет, только показывает по просьбе.
5. **⭐ Ключевые идеи и высказывания (отдельный документ + лёгкая база — с перечитыванием).** *Новое
   по запросу Руслана, отдельно от философского листа.* Если философский лист — связная личная
   философия (проза), то здесь — **коллекция отдельных мыслей**: цитаты, ключевые идеи, важные
   направления, высказывания. Две части: **(а)** топ **3-5 ключевых сейчас** наверху (фокус, держишь
   перед глазами); **(б)** лёгкая база Wisdom/Ideas (Text · Source · Kind: цитата/идея/направление/
   принцип · Tag · ⭐ Key now · Added), куда сыплешь много. **Механика перечитывания:** вид
   «⭐ Ключевые сейчас» (топ 3-5) + вид «🎲 Случайная мудрость» (resurface 1-3 случайных) + строчка
   в weekly review «прочитай 3 случайных» + опц. CC в wrap-of-day подкидывает одну. Логика как
   «посмотреть позже» в Knowledge: добавить легко — **и достать обратно тоже легко**, иначе копить
   незачем.

**Важно:** всю стратегическую и философскую прозу (Vision, смысл целей, точку Б, философский лист,
ключевые идеи) **пишет и курирует человек**. CC только предзаполняет данные в базе Goals, подтягивает
цифры и **показывает** философский лист / коллекцию по просьбе (включая resurface) — «куда я иду»,
«как хочу жить» и «что для меня мудро» решает только человек (правило конституции №1).
...
- **🗓️ Weekly review (воскресенье, ~20 мин):** Что сделал за неделю (топ-5, CC предзаполняет из
  ...
  ⭐ перечитал 3 случайных из копилки идей.
- **📆 Monthly review (конец месяца, ~45 мин):** Проекты обзор → Цели: прогресс → Финансовый пульс →
  Контакты: аудит касаний → Привычки: что держалось → 🧠 Философский лист: что понял → Паттерны и уроки.
...
| **# баз (baseline)** | 10 (вкл. Habits) (+ Strategic под-раздел + 2 ревью) | Generic: ~7 баз/слотов; Jetix overlay поверх | 12 групп баз (4 ядро) + Hypotheses + Vision/Goals |
...
| **Новое в v2** | усиленный Daily Log + Habits база + Strategic (Vision/Goals/Точка А→Б + Философский лист + Ключевые идеи) + 2 ревью + «посмотреть позже» | Generic/Jetix split + Brand pattern | Hypotheses + Vision/Goals страницы + L1↔L3 connect |
