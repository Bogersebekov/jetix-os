---
title: Voice Pipeline — публичное описание V2 EXTENDED (voice-pipeline + метод overview + Jetix overview + curated links для методологов + AI assistant browsing)
date: 2026-05-26
...
audience: Церен (Tseren) + его AI assistant (IWE, пройдётся по curated links) + любой методолог-исследователь-изобретатель + open community
...
phase: Phase 7 EXTENDED (Main + §K метод + §L Jetix overview + §M curated links + §N JM-1..JM-3 mermaid)
F: F2-F3
G: voice-pipeline-public-v2-phase-7-extended
constitutional_posture: R1 surface only + R2 STRICT + R6 honest inheritance + R11 + R12 paired-frame STRICT + IP-1 + append-only
language: russian primary
status: DRAFT — Ruslan R1 prose pass + решение о публикации
length_target: 12-15K plain Russian
mermaid_count: 7 (VP-1..VP-4 voice-pipeline + JM-1..JM-3 метод/Jetix overview, все inline)
phase_reports:
...
> (явный запрос «1-2 страницы — на текст отвечу») и его AI-ассистента, любого методолога, который
> заглянет в репозиторий, и community для воспроизведения.
>
> **EXTENDED-версия.** Помимо самого voice-pipeline (§A-§G) документ даёт короткие overview двух
> вещей, в которых pipeline живёт: **§K — метод** (мастерство = собрать нужные методы + применить
> нужный в нужный момент) и **§L — что есть Jetix** (16 направлений + Foundation triad). Плюс **§M —
> curated links**: список главных документов с описаниями, чтобы ваш AI-ассистент (IWE или другой)
> мог пройтись по репозиторию сам. Это сделано осознанно: один документ → можно прочитать самому ИЛИ
> отдать ассистенту для deeper exploration.
>
> **Как читать.** 5 минут — §0 TL;DR. 25-30 минут — весь main (§A-§Q). Только voice-pipeline —
> §A-§G + §O. Только «что за всем этим стоит» — §K + §L + §M. Глубже — 7 phase-report'ов в
> `reports/voice-pipeline-public-v2-2026-05-26/`. Для письма — `00-SUMMARY-FOR-TSEREN.md` (≤700 слов).
> Самое важное в самом pipeline — **§C двойная фильтрация**.
>
...
| Документ | Что внутри | Время | Кому |
|---|---|---|---|
| **этот документ** | Voice Pipeline public V2 EXTENDED — то, что вы читаете | — | старт |
...
*Document closure 2026-05-26 (Phase 7 EXTENDED 2026-05-27). Voice Pipeline Public V2 EXTENDED — deep
version + метод overview (§K) + Jetix overview (§L) + curated links для AI-ассистента (§M). F2-F3
derivative (existing system documentation + Ruslan voice 26-27.05 + реальный код tools/ + Method V2 §J +
V4 metaplan substrate). **7 mermaid inline:** VP-1..VP-4 (voice-pipeline) + JM-1..JM-3 (метод-дерево /
16 направлений / путь чтения). 7 phase-report'ов + SUMMARY ≤700w + diagrams INDEX. Honest inheritance
(Дарвин/да Винчи/Луман/Форте/Карпатый для pipeline; Левенчук-OMG-MMK/Ericsson/Dweck/восточные традиции
для метода) — НЕ «революция»; свой вклад честно небольшой (synthesis + AI-augmentation + Workshop frame
+ Mastery at transitions + Templates × Unique + Preparation Stage explicit). Personal context: год+
работы, 1100+ заметок до Jetix-репо (acceleration ≠ initiation). Технический стек сверен с кодом
(CC-headless backend / whisper-large-v3 / 2 human-gate / двойная фильтрация). R1 surface (финальная
prose + публикация = Ruslan). R2 STRICT (Foundation untouched — §K/§L только references, ноль записей в
LOCKED paths). R6 honest inheritance. R11 (NO auto-actions; релиз = ack). R12 paired-frame STRICT
(substance-only, fork-friendly, open-source, anti-marketing). IP-1 (Ruslan = sole author final;
brigadier = substrate compile). Append-only (Phase 7 EXTENDED расширяет тот же V2 файл; 7 phase-report'ов
preserved untouched). Supersedes first Cloud Cowork DRAFT + simple Phase 7. Pool result — NO auto-launch
consequent.*
