...
### 1.6 ⭐ Jetix Private Library + Knowledge-as-Leverage (stratified moat)

**Критическая стратегическая концепция, учитывай при design.**

**Проблема:** AI обучен на интернет-мусоре → даёт мусорные ответы. Конкуренты
limited mainstream AI. Jetix конкурентное преимущество = curated качественная
база знаний + новая философия работы с информацией.

**Jetix Private Library — что это:**
1. **Первоисточники** (книги / методологии / research / case studies) curated и
   собранные в единый private chan. Jetix платит за качественные материалы.
   Партнёры тоже покупают и добавляют.
2. **Closed-core** (Lock 13) — закрыта от external слоя. Partner-tier access
   через matchmaker (Lock 21).
3. **Доступ через Karpathy LLM OS pattern + wiki/** — быстрый адекватный
   retrieval для agents и human users.

**Новая философия работы с информацией — "pool first, query second":**
- Раньше: люди искали инфу → изучали → применяли
- Теперь: качественная инфа УЖЕ в pool'е (отфильтрована, curated) → люди
  задают качественные вопросы → получают качественные ответы
- Jetix как **platform коммуникации умных людей + agregator лучших идей** —
  researchers / investors / entrepreneurs sharing curated knowledge

**Design implications для роя:**
- **Private library = first-class architectural component**, не просто «wiki/
  папка». Это revenue-generating asset Phase 2+
- Quality связей важнее количества — provenance / citation / first-source
  fidelity mandatory
- Access layer — fast retrieval via LLM wiki pattern (Karpathy endorsed)
- Permissions layer — tiered access (core/partner/member/public)
- Contribution workflow — как партнёры добавляют materials, validation gates
- Curation mechanisms — /lint + expert review для rot prevention
- Monetization design Phase 2+ — partner access = membership tier

**Knowledge-as-leverage — главный ров (moat):**
Jetix answers качественнее на порядок при тех же вопросах. Это позволяет
быстро и адекватно развиваться независимо от внешних ресурсов. Конкуренты
на mainstream AI не могут догнать.

**Связь с Locks:**
- Lock 13 (open-surface / closed-core) — library = closed-core
- Lock 20 (USB-C positioning) — library как standards-level knowledge protocol
- Lock 24 (open-source research Phase 2+) — часть research outreach (public),
  core private
