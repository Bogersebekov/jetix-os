...
### 2026-04-25 — Peer-input parallel cell pattern validated + template-level `cell_severity` class (cyc-layer-6-community-deep-dive)

- **Decision:** Two new patterns compound-validated across cycle-5:
  (1) **Peer-input parallel cell pattern** — when a primary integrator cell requires formal arithmetic/financial dimension out-of-lens (systems-expert Alliance structure analysis lacks unit-econ; systems-expert matchmaker design lacks KPI conversion), dispatch investor-expert (or other appropriate lens-holder) **peer cell in same wave** producing sibling draft the integrator cell references but does NOT duplicate. Primary cell frontmatter carries `<lens>_peer_input: <path>`; inline cross-draft citation handles integration. Brigadier integration cell folds peer content into canonical document.
  (2) **Template-level `cell_severity` class** — Ruslan directive to de-prioritize specific sections (§8 Membership + §9 Growth «template-level по приколу») operationalized as explicit word hard-cap in `cell_acceptance_predicate:` with reject-on-exceed enforcement. Cells self-police. Pattern: `cell_acceptance_predicate: target_words: 400-800 HARD CAP; exceeds = reject`.
- **Reasoning:** Pattern (1) validated — cycle-5 had 2 investor peer cells (S5-ROI 3414w + S6-KPIs 3961w) alongside systems-expert primary cells (S5 Alliance 7843w + S6 Matchmaker 4052w). Zero `peer-input-needed` escalations. Integration cell (brigadier) references peer arithmetic via footnotes; canonical document contains full arithmetic via inline summary without duplication. Investor conclusions (Option C max capital-efficiency Phase-1) directly informed brigadier Option C recommendation → Ruslan first-read ack. Pattern (2) validated — §8 landed 633w, §9 landed 597w, both within 400-800 cap. Mgmt-expert self-policed by brief explicitly stating «HARD CAP — exceeds = reject» — zero brigadier edit-passes needed for word reduction.
- **Result:** Cycle-5 = first Phase-2 layer-deep-dive. Two new patterns compound on top of cycle-4 description-shape M-task (4th task-shape) + cycle-3 design-record→extraction pattern + cycle-3 critic-in-parallel+proposed_replacement + cycle-3 Option-B self-selection. **Pattern stack** now provides layer-deep-dive M-task template: 13-15 cells across 3 waves; integrator-dominant primary cells + peer-input cells for out-of-lens arithmetic; Wave-C philosophy-critic dual cells (§Open-Qs + §Dissents); template-level `cell_severity` for Ruslan-deprioritized sections; brigadier integration assembles canonical 15-25K-word document.
- **Review:** partial — each pattern operative on 1 cycle (cycle-5 for peer-input + cell_severity). Propose for brigadier §3.2 shared-protocols update: (a) add `peer-input` as formal dispatch mode (not 5th matrix mode, which would break 5×4=20 invariant — add as auxiliary dispatch pattern like `mode: writing-support` in shared-protocols §7); (b) add `cell_severity: {primary | template-level}` field to WBS cell schema with corresponding word-cap enforcement rules. Cycle-6 (L5) + cycle-7 (L7) should validate both patterns; if validated, lock in Phase-B shared-protocols revision.
