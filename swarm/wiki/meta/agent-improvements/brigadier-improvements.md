...
### 2026-04-24 — brigadier-improvement from cyc-km-materialization Wave 1: design-record vs physical-extraction two-stage pattern for implementation M-tasks

- **Decision:** For implementation M-class tasks (outputs are files, not decisions), brigadier dispatches in 2 stages: Stage-A (Wave 1) = authoritative design records per Part under `swarm/wiki/designs/<task-id>/`; Stage-B (Wave 2+) = extraction of physical files from design records into `.claude/`, `tools/`, `swarm/wiki/_templates/`, `swarm/wiki/operations/`. Stage-A cells write ONE draft per Part. Stage-B cells read the promoted design record + spawn N physical files via Write operations. Brigadier integration budget for Stage-A = O(draft reads + gate checks), NOT O(file count). This prevents brigadier from hitting context-limit cliff mid-extraction.
- **Reasoning:** T-km-materialization-mvp-2026-04-24 Part A/B/C design records embed specs for ~22 physical files across 7 file trees. Attempting single-brigadier-context extraction would (a) blow context budget, or (b) force resume-state handoff with incomplete state. The two-stage pattern aligns with the existing `drafts/ → canonical/` discipline (shared-protocols §1) and extends it to `designs/ → physical-extraction/`.
- **Result:** Wave-1 promoted 4 drafts (~300 KB) with minimal brigadier-context cost (~5K tokens per promotion: frontmatter update + commit message). Part A/B/C extraction queued for Wave-2 engineering × integrator cell with fresh context window (30K ap_cost / 50K ap_budget).
- **Review:** validated this cycle. **Proposed protocol addition to `.claude/agents/brigadier.md §3.3`:** when task-class is M-structural AND shape includes "implementation" (physical-file output expected), default decomposition adds a Stage-B extraction cell per Part's design record. Cell brief format: "Read `<design-record-path>`. For each §N.N file block in the body, Write the file to its target path as declared in the block's 'Target path:' line. Commit atomically. Do not alter content."

### 2026-04-24 — brigadier-improvement from cyc-km-materialization Wave 1: critic-in-parallel beats critic-after for rigor-constrained artefacts

- **Decision:** When a Wave-1 integrator cell writes artefacts under a formal rigor constraint (binary/typed/schema-bound predicates; e.g., Part B SG predicates under Hamel-binary DSL-canonical form), brigadier dispatches the rigor critic IN PARALLEL with the integrator, not after. Brigadier applies critic fixes during integration as Edit ops on the integrator's draft — NO author-cell re-dispatch. Preserves M-class budget (no retry-hit), respects E-15 (rewrites are critic-originated, not brigadier-originated), minimises wall-clock.
- **Reasoning:** Wave-1 this cycle: philosophy × critic on Part C ran in parallel with mgmt × integrator on Part B and systems × integrator on Part C. Critic returned 14 FAIL findings before either integrator promotion gate ran. Brigadier applied fixes from critic's `proposed_replacement:` fields directly (see philosophy-expert-improvements 2026-04-24 entry). Net: zero §5.5.5 gate rejections; zero retry cycles; ~38 min wave wall-clock total.
- **Result:** Wave-1 gate-pass on first attempt for all 4 drafts. Philosophy-critic's proposed_replacement entries were machine-actionable — brigadier's apply-phase was mechanical.
- **Review:** validated. **Proposed protocol addition to `.claude/agents/brigadier.md §3.2` (task-shape → cell-selection matrix):** for task-shape "design-with-formal-rigor-constraint", default cell-set adds `<rigor-expert> × critic` in Wave 1 (parallel with primary integrator). Triggers include: SG predicates (philosophy × critic), schema-bound frontmatter (engineering × critic), capital arithmetic (investor × critic), loop-dominance claims (systems × critic). Reduces integration friction from O(N findings) retry-cycles to zero.
