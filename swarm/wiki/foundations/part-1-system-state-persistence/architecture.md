...
| **Offline guarantee test stub (P1.3 new)** | F3 (Wave D OQ-B1-7 ack) | `strace -e trace=network` path chosen (unshare -n confirmed unavailable 2026-04-28; AppArmor sandbox). P1.3 acceptance predicate satisfiable via strace fallback. [src:decisions/RUSLAN-ACK-WAVE-D-INTEGRATION-PASS-2026-04-28.md §2.2; src:K10 RESOLVED note] | Holds via strace path; verified 2026-04-28 | R-medium — strace path confirmed available; first end-to-end test promotes to F4 | Promote to F4 when `tools/tests/test-offline-guarantee.sh` passes end-to-end with strace path |
...
**K10 — `unshare -n` unavailable on prod server (P1.3 HARD GAP — RESOLVED 2026-04-28 Wave D OQ-B1-7).**
Detection (verified 2026-04-28): `unshare -n echo test` exits with "unshare failed: Operation not permitted" on Ubuntu 24.04.3 LTS / kernel 6.8 (AppArmor restricts unprivileged namespace creation). **Resolution per Ruslan ack Wave D AWAITING-APPROVAL §4.2 OQ-B1-7:** P1.3 acceptance predicate **satisfied via `strace -e trace=network` fallback path** (chosen path). `tools/tests/test-offline-guarantee.sh` uses `strace -e trace=network <command> 2>&1 | grep -qE 'connect|socket'` as primary mechanism (strace verified installed); `unshare -n` and `LD_PRELOAD` documented as historical alternatives only. P1.3 acceptance predicate now satisfiable end-to-end. [src:decisions/RUSLAN-ACK-WAVE-D-INTEGRATION-PASS-2026-04-28.md §2.2 OQ-B1-7]
