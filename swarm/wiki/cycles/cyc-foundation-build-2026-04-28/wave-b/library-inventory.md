...
## §10 2026-04-27 Wave B Supplement additions

Following Ruslan walkthrough of cycle 12 wave A+B AWAITING-APPROVAL packet, 5 fundamental sources were ingested via `tools/convert/convert_all.py` to lift 3 F4 consultant cards (#5, #6, #13). All 5 converted successfully with quality grades; new subcategories `anthropic/`, `event-sourcing/`, `sre/` created.

| # | Source | Path | Grade | Words | Card lifted | F-level shift |
|---|--------|------|-------|------:|-------------|---------------|
| 1 | Bai et al. 2022 — *Constitutional AI: Harmlessness from AI Feedback* (arxiv:2212.08073) | `raw/books-md/anthropic/bai-2022-cai.md` | A | 19,289 | #13 CAI | F5 web-only → F5 library-direct |
| 2 | Askell et al. 2021 — *A General Language Assistant as a Laboratory for Alignment* (arxiv:2112.00861) | `raw/books-md/anthropic/askell-2021-hhh.md` | A | 24,915 | #13 CAI | F5 web-only → F5 library-direct (HHH framework canonical) |
| 3 | Greg Young — *CQRS Documents* (2010, cqrsinfo.com) | `raw/books-md/event-sourcing/young-cqrs-2010.md` | A | 17,520 | #5 Event Sourcing | F4 training-knowledge → F4-F5 (1/5 library-direct) |
| 4 | Google SRE Workbook Ch. 2 — *Implementing SLOs* (Thurgood/Ferguson/Hidalgo/Beyer, 2018) | `raw/books-md/sre/google-srewb-implementing-slos.md` | B | 8,744 | #6 SRE | F3 training-knowledge → F5 library-direct |
| 5 | Beyer/Jones/Petoff/Murphy (eds.) — *Site Reliability Engineering: How Google Runs Production Systems* (O'Reilly, 2016) | `raw/books-md/sre/google-sre-book.md` | A | 190,412 | #6 SRE | F3 training-knowledge → F5 library-direct (full SRE Book) |

**Original sources** (PDFs/TXT) were located in `inbox/{anthropic,event-sourcing,sre}/` during conversion (consumed/processed by pipeline; pipeline does not retain raw inputs after successful conversion — provenance trail is in the converted markdown frontmatter `acquired_date`/`converted_date`/`converted_via: docling`).

**INDEX.md** regenerated by pipeline; total content files now **402** (was 391 — net +11 to account for 5 new files plus 6 prior pending which were not in INDEX at last validation; conservative count after this run is the authoritative number going forward).

**Library coverage post-supplement:**
- Card #5 Event Sourcing — 1/5 canonical library-direct (Greg Young 2010); Kleppmann DDIA / Fowler / Vernon / Dahan still training-knowledge.
- Card #6 SRE — 2/5 canonical library-direct (SRE Book full + SLO Workbook chapter); Honeycomb Observability Engineering / Mike Julian Practical Monitoring / OpenTelemetry spec still training-knowledge.
- Card #13 CAI — 2/5 library-direct (Bai 2022 + Askell 2021); Anthropic RSP v1.2 + Model Specification still web-only.

Card #14 Mereology F4 academic remains unchanged — Ruslan deferred mereology supplement (low criticality for Foundation phase).
